...
var_x_curve = hv.Curve(
    (steps, history["var_x"]), kdims=["Step"], vdims=["Position Variance"], label="V_Var,x(t)"
).opts(
    width=900,
    height=400,
    color="blue",
...
var_v_curve = hv.Curve(
    (steps, history["var_v"]), kdims=["Step"], vdims=["Velocity Variance"], label="V_Var,v(t)"
).opts(
    width=900,
    height=400,
    color="green",
...
alive_curve = hv.Curve(
    (steps, history["n_alive"]), kdims=["Step"], vdims=["Alive Walkers"], label="Alive"
).opts(
    width=900,
    height=300,
    color="darkgreen",
...
high_error_curve = hv.Curve(
    (steps, history["n_high_error"]), kdims=["Step"], vdims=["Count"], label="High-Error"
).opts(color="red", line_width=2)

low_error_curve = hv.Curve(
    (steps, history["n_low_error"]), kdims=["Step"], vdims=["Count"], label="Low-Error"
...
com_trajectory = hv.Path([history["mu_x"]], kdims=["x", "y"], label="COM Trajectory").opts(
    color="purple", line_width=2, alpha=0.7
)

start_point = hv.Scatter(([history["mu_x"][0, 0]], [history["mu_x"][0, 1]]), label="Start").opts(
    marker="o", size=12, color="green"
)

end_point = hv.Scatter(([history["mu_x"][-1, 0]], [history["mu_x"][-1, 1]]), label="End").opts(
    marker="x", size=15, color="red", line_width=3
)

optimum = hv.Scatter(([x_opt[0].item()], [x_opt[1].item()]), label="Optimum").opts(
    marker="star", size=20, color="gold"
)
...
def create_swarm_plot(step):
    ...
    if dead.any():
        plots.append(
            hv.Scatter((x[dead, 0], x[dead, 1]), label=f"Dead ({dead.sum()})").opts(
                size=6, color="gray", alpha=0.3
            )
        )
    ...
    if L.any():
        plots.append(
            hv.Scatter((x[L, 0], x[L, 1]), label=f"Low-Error ({L.sum()})").opts(
    ...
    if H.any():
        plots.append(
            hv.Scatter((x[H, 0], x[H, 1]), label=f"High-Error ({H.sum()})").opts(
    ...
    if arrows:
        arrow_plot = hv.Segments(arrows).opts(color="cyan", alpha=0.5, line_width=1.5)
        plots.append(arrow_plot)

    # Center of mass
    mu = history["mu_x"][step]
    com_point = hv.Scatter(([mu[0]], [mu[1]]), label="COM").opts(
        marker="+", size=20, color="purple", line_width=3
    )
    plots.append(com_point)

    # Optimum
    opt_point = hv.Scatter(([x_opt[0].item()], [x_opt[1].item()]), label="Optimum").opts(
        marker="star", size=15, color="gold"
    )
...
print("\n🔍 Key Observations:")
print(
    f"  • Position variance reduced by {(1 - history['var_x'][-1] / history['var_x'][0]) * 100:.1f}%"
)
print(
    f"  • Velocity variance reduced by {(1 - history['var_v'][-1] / history['var_v'][0]) * 100:.1f}%"
)
print(
    f"  • Center of mass converged to within {np.linalg.norm(history['mu_x'][-1]):.4f} of optimum"
)
