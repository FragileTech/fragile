...
WalkerState WalkerState::clone(const std::vector<int32_t>& companions,
  ...
  if (!lineage.empty()) out.lineage = gather_clone(lineage, companions, will_clone);
  return out;
}
...
void WalkerState::inject(const WalkerState& source, int32_t count) {
  ...
  for (int32_t i = 0; i < count; ++i) {
    ...
    if (!lineage.empty() && !source.lineage.empty()) lineage[ui] = source.lineage[ui];
  }
}
...
WalkerState WalkerState::extract(const WalkerState& s,
  ...
  if (!s.lineage.empty()) out.lineage.resize(indices.size());
...
WalkerState WalkerState::concat(const WalkerState& a, const WalkerState& b) {
  ...
  if (!a.lineage.empty() && !b.lineage.empty()) cat(out.lineage, a.lineage, b.lineage);
  return out;
}
