...
class FractalCloningOperator {
 public:
  ...
  std::vector<float> fitness_with_companions(
  ...
  std::vector<float> clone_probs_with_companions(
      const std::vector<float>& virtual_rewards,
      const std::vector<int32_t>& companions) const;

  /// Decision with uniforms given: will_clone = (p > u) | dead.
  std::vector<uint8_t> decide_with_uniforms(
      const std::vector<float>& clone_probs,
