...
class ExplorationTree {
 public:
  ...
  uint32_t append(uint32_t parent, uint32_t frames, const float* action, const float* pose,
  ...
  void export_data(std::vector<uint32_t>& metadata, std::vector<float>& values) const;
 ...
 private:
  ...
  std::vector<uint32_t> prune_leaves_;
  std::vector<uint8_t> prune_pins_;
