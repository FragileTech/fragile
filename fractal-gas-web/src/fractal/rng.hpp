...
class Rng {
 public:
  ...
  void permutation_into(int32_t n, std::vector<int32_t>& p) {
    p.resize(n);
...
class Mt19937Rng final : public Rng {
 public:
  ...
  void restore(const std::string& state) {
    ...
    std::mt19937_64 next;
    if (!(in >> next)) throw std::invalid_argument("Invalid planner RNG state");
    in >> std::ws;
    if (!in.eof()) throw std::invalid_argument("Trailing planner RNG data");
    gen_ = next;
  }
