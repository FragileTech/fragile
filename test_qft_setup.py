...
def test_quadratic_well_benchmark():
    ...
    assert torch.allclose(result_test, torch.full((10,), expected)), f"Expected {expected}, got {result_test[0]}"
...
def test_qft_config_creation():
    ...
    assert config.benchmark_name == "Quadratic Well", f"Expected 'Quadratic Well', got {config.benchmark_name}"
    ...
    assert config.kinetic_op.epsilon_F == 38.6373, f"Expected 38.6373, got {config.kinetic_op.epsilon_F}"
    ...
    assert config.companion_selection.epsilon == 2.80, f"Expected 2.80, got {config.companion_selection.epsilon}"
    assert config.companion_selection_clone.epsilon == 1.68419, f"Expected 1.68419, got {config.companion_selection_clone.epsilon}"
...
def test_potential_creation():
    ...
    assert isinstance(config.potential, QuadraticWell), f"Expected QuadraticWell, got {type(config.potential)}"
