...
The C³ regularity analysis ([c3_adaptative_gas.md](c3_adaptative_gas.md)) established that the fitness potential $V_{\text{fit}}[f_k, \rho](x_i)$ is three times continuously differentiable with k-uniform and N-uniform bounds. This result validated the BAOAB discretization theorem, which requires $V \in C^3$ for $O(\Delta t^2)$ weak error.

However, several theoretical and practical motivations drive us to establish **C⁴ regularity** (four times continuous differentiability):

**1. Higher-Order Functional Inequalities**

Certain powerful functional inequalities require C⁴ regularity:
- **Brascamp-Lieb inequality**: Requires twice-differentiable densities (equivalent to $C^4$ potentials)
- **Bakry-Émery calculus**: Γ₂-calculus for hypercontractivity uses fourth derivatives
- **Log-Sobolev inequalities with optimal constants**: Sharper bounds require C⁴ smoothness

**2. Higher-Order Integrators**

While BAOAB achieves $O(\Delta t^2)$ weak error with C³ regularity, higher-order splitting schemes exist:
- **BABAB integrator**: Achieves $O(\Delta t^4)$ but requires $V \in C^5$
- **Runge-Kutta schemes**: Fourth-order methods need $C^4$ or $C^5$
- **Modified Hamiltonian methods**: Backward error analysis benefits from smoother potentials

**3. Perturbation Theory Refinement**

The adaptive force $\mathbf{F}_{\text{adapt}} = \epsilon_F \nabla V_{\text{fit}}$ appears as a perturbation to the Euclidean backbone. Higher-order smoothness enables:
- Sharper bounds on perturbation size via Taylor expansions
- Better understanding of stability under parameter variations
- Refined estimates for the critical perturbation threshold $\epsilon_F^*$

**4. Hessian Lipschitz Continuity**

C⁴ regularity implies that the Hessian $\nabla^2 V_{\text{fit}}$ is Lipschitz continuous:

$$
\|\nabla^2 V_{\text{fit}}(x) - \nabla^2 V_{\text{fit}}(y)\| \le L_4 \|x - y\|
$$

where $L_4 \le \|\nabla^3 V_{\text{fit}}\|_{\infty}$. This Lipschitz property is crucial for:
- **Newton-Raphson convergence**: Guarantees quadratic convergence rate
- **Cubic regularization**: Trust-region methods with optimal complexity
- **Gradient flow regularity**: Smoother trajectories in optimization dynamics

### 1.2. Main Result Preview

:::{prf:theorem} C⁴ Regularity (Preview)
:label: thm-c4-preview

Under suitable C⁴ assumptions on the measurement function $d$, localization kernel $K_\rho$, rescale function $g_A$, and regularized standard deviation $\sigma'_{\text{reg}}$, the fitness potential

$$
V_{\text{fit}}[f_k, \rho](x_i) = g_A\left( \frac{d(x_i) - \mu_\rho[f_k, d, x_i]}{\sigma'_{\text{reg}}(\sigma^2_\rho[f_k, d, x_i])} \right)
$$

is **four times continuously differentiable** with k-uniform and N-uniform bound:

$$
\|\nabla^4_{x_i} V_{\text{fit}}[f_k, \rho](x_i)\| \le K_{V,4}(\rho) < \infty
$$

for all alive walker counts $k \in \{1, \ldots, N\}$ and all swarm sizes $N \ge 1$.

Moreover, the fourth derivative bound scales as:

$$
K_{V,4}(\rho) = O(\rho^{-3}) \quad \text{for } \rho \to 0
$$

:::

This document provides the complete proof of this theorem and analyzes its implications for the adaptive algorithm.

### 1.3. Document Organization

- **Section 2**: Mathematical framework and notation (extending C³ setup)
- **Section 3**: C⁴ regularity assumptions for all components
- **Section 4**: Fourth derivatives of localization weights (telescoping identities)
- **Section 5**: Fourth derivatives of localized moments (mean and variance)
- **Section 6**: Fourth derivative chain rule for regularized standard deviation
- **Section 7**: Fourth derivative of the Z-score (quotient rule)
- **Section 8**: Main C⁴ regularity theorem (composition via chain rule)
- **Section 9**: Stability implications and corollaries
- **Section 10**: ρ-scaling analysis and numerical considerations
- **Section 11**: Comparison with C³ analysis and practical guidance
...
**Scope of analysis:** This document analyzes a **simplified fitness potential model** where the measurement function $d: \mathcal{X} \to \mathbb{R}$ depends only on a walker's position $x_i$. In the full adaptive gas framework (see [07_adaptative_gas.md](../07_adaptative_gas.md)), the diversity measurement $d_i = d_{\text{alg}}(i, c(i))$ depends on the entire swarm state through companion selection $c(i)$. Extending the C⁴ analysis to the full swarm-dependent measurement is left as future work.
...
This refined bound (with the square-root exponent in the Gaussian) is proven by writing derivatives as polynomials in $(x_i - x_j)/\rho$ times the kernel (see [c3_adaptative_gas.md](c3_adaptative_gas.md) §5 for details at lower orders; the pattern extends to all $m$).
...
where $K_{V,3}(\rho) = O(\rho^{-3})$ is the third-derivative bound from C³ regularity (see {prf:ref}`thm-c3-regularity` in [c3_adaptative_gas.md](c3_adaptative_gas.md)).
...
- **C³**: $\|\nabla^3 V_{\text{fit}}\| = O(\rho^{-3})$ (third derivative has cubic singularity - see [c3_adaptative_gas.md](c3_adaptative_gas.md))
...
- $K_{Z,3}(\rho) = O(\rho^{-3})$ (third derivative from C³ analysis—see [c3_adaptative_gas.md](c3_adaptative_gas.md))
