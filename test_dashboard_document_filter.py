...
def test_document_filter():
    ...
    import panel as pn
    assert isinstance(dashboard.document_filter, pn.widgets.MultiChoice), \
    ...
    if dashboard.document_filter.options:
        ...
        print(f"   ✓ Document filter populated with options")
    else:
        print(f"   ⚠ No documents found (registry may be empty or example)")

    # Check that all documents are selected by default
    assert set(dashboard.document_filter.value) == set(dashboard.document_filter.options), \
        "All documents should be selected by default"
    print(f"   ✓ All documents selected by default")

    # Check that _update_document_filter method exists
    assert hasattr(dashboard, '_update_document_filter'), \
