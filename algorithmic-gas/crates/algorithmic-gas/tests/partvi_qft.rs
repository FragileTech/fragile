...
fn run(id: u32, p: serde_json::Value) -> ExperimentResult {
    partvi::analyze(&ExperimentRequest {
        experiment: id,
        parameters: p,
    })
    .unwrap()
...
fn archive_color_and_twistors_use_executed_stages_and_companion_slots() {
    ...
    futures_lite::future::block_on(async {
        ...
        .unwrap();
        for fit in no_fit.details["fits"].as_array().unwrap() {
            assert_eq!(fit["status"], "unavailable");
            assert_eq!(fit["mass"], serde_json::Value::Null);
        }
    });
}
