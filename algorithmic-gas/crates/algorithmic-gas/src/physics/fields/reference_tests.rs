...
fn run(experiment: u32, parameters: Value) -> ExperimentResult {
    ...
    match experiment {
        37..=44 => gravity::run(&request, None),
        45..=51 => models::run(&request, None),
        52..=60 => boundary::run(&request, None),
        _ => cosmology::run(&request, None),
    }
    .unwrap_or_else(|e| panic!("reference oracle {experiment}: {e}"))
}
...
fn fastest_advertised_density_mode_preserves_positive_decay() {
    let r = run(
        47,
        json!({"wave_number":6,"length":0.5,"diffusion":0.2,"rate":2.,"resolution":256}),
    );
