...
The runner accepts `--benchmark ID` (any catalog id; `--list-benchmarks` prints the catalog), `--instance N` for COCO BBOB, `--param KEY=VALUE` for benchmark parameters, `--dimensions`, `--seed`, `--backend`, and `--config FILE`. Arguments apply in order; place `--config` before overrides. Explicit file boundary settings are preserved. Output is JSON with resolved configuration, initialization and step timings, raw reward summaries, submitted reward-row count, synchronization count and transfer bytes. Initialization/compilation must be reported separately from steady-state performance: `initialization_seconds` covers the build, `first_step_seconds` the first execution of every kernel, and `steady_state_seconds` / `steady_state_steps_per_second` the remaining steps. A failed step commits nothing; the runner still writes the archive of the committed steps and the summary, with the failure under `error`, and exits with that error.
...
| Sampling law | Independent directed draws; uniform Fisher–Yates mutual matching; sequential Gaussian greedy matching with an ascending (default) or random pivot order; permutation |
| Diversity reduction | Mean, weighted mean, minimum, maximum of scalar pair comparisons |

Distance companions are dense masked `[N,K]` pool indices, default `K=1`. Multiple mutual rounds produce multiple companions; each round is a matching, not one globally disjoint multi-edge graph. Greedy matching pivots on the first remaining slot in ascending slot id; `pivot: "random"` selects an independent uniform pivot permutation, which is a different joint law. An active walker left without a valid distance donor (the odd walker under `odd: "unmatched"`) has no diversity measurement: the reducer returns a topology error and never reports zero separation for it. Cloning currently requires `K=1`. `ClonePlan` records a list of weighted sources per recipient so future recombination can extend the representation, but more than one contributing donor is explicitly rejected today.

Without replacement, `insufficient: "use_available"` is the default: remaining capacity is masked when fewer candidates survive. Rows sampled without replacement are in draw order for both the uniform and the kernel-weighted law, so entry `a` of a row is its `a`-th sequential draw and positional `weighted_mean` weights apply to draws. `"reject"` selects a strict error. Uniform replacement draws use direct ranks; uniform sampling without replacement uses a sparse partial shuffle. Mutual Fisher–Yates uses one precomputed slot lookup, so its single-round sampling work is linear.
...
Fitness separates measurement, standardization, positive mapping and combination. Built-ins include global alive-only regularized population statistics, local kernel-weighted statistics, sample statistics, logistic and asymmetric maps, additive positivity floors, independent reward/diversity exponents, and a distance measurement floor. A channel whose exponent is zero contributes one and its positive map is not evaluated. The distance floor applies **after** multi-companion reduction as `sqrt(reduced² + floor²)`.
