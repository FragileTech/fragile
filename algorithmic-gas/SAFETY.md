...
- The tessellation stage is safe Rust: the 3D Delaunay structure is an index arena with checked adjacency, its predicates (`robust`) and Spade's are exact, and structural inconsistencies surface as `GasError::Numerical` instead of undefined behavior. Lifted cliques of coincident walkers are counted against `max_batch_elements / 2` neighbor edges before any edge is materialized, the stage reserves a linear working set against the remaining memory allowance, and restored `GraphSnapshot`s are validated (CSR layout, reverse index, array lengths) before use. Rayon workers receive disjoint output slices; the `parallel` feature adds no shared mutable state.
