...
def example_3_batch_processing():
    ...
    results = process_multiple_documents(
        file_paths=files_to_process, chapter="1_euclidean_gas", model="claude-sonnet-4"
    )
...
def example_4_custom_enrichment():
    ...
    enriched_theorem = TheoremBox.from_raw(
        raw_theorem, chapter="1_euclidean_gas", document="example_document"
    )
    ...
    enriched_axiom = Axiom.from_raw(
        raw_axiom, chapter="1_euclidean_gas", document="example_document"
    )
...
def main():
    ...
    parser.add_argument(
        "--example", type=int, choices=[1, 2, 3, 4, 5], help="Run a specific example (1-5)"
    )
    parser.add_argument("--file", type=str, help="Process a specific file (for Example 1)")
