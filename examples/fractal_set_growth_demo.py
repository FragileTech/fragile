...
def create_spatial_trajectory_plot(result, N):
    ...
    initial_scatter = hv.Scatter(initial_points, kdims=['x', 'y'], vdims=['walker_id', 'type']).opts(
        color='green',
        size=10,
        marker='o',
        alpha=0.6,
...
def create_spacetime_plot(nodes_df, edges_df, up_to_timestep=None):
    ...
    nodes_scatter = hv.Points(
        nodes_spacetime,
        kdims=['x', 't'],
        vdims=['walker_id', 'timestep', 'node_id', 'high_error']
    ).opts(
        color='walker_id',
        cmap='Category10',
        size=6,
        ...
        width=800,
        height=600,
        title=f'FractalSet Spacetime Diagram (x, t)',
        xlabel='x position',
        ylabel='Time',
    )
    elements.append(nodes_scatter)
...
def create_statistics_plot(fs, n_steps):
    ...
    for t in range(n_steps + 1):
        ...
        stats.append({
            ...
            'n_kinetic': n_kinetic,
            'n_cloning': n_cloning,
            'variance': fs.graph.graph['var_x_trajectory'][t] if t < len(fs.graph.graph['var_x_trajectory']) else 0,
        })
...
def main():
    ...
    gas, result, fs = run_gas_with_fractal_set(N=N, d=d, n_steps=n_steps)

    print(f"FractalSet built with {fs.graph.number_of_nodes()} nodes and {fs.graph.number_of_edges()} edges")
    ...
    dashboard = pn.Column(
        pn.pane.Markdown("# FractalSet Growth Visualization\n\n"
