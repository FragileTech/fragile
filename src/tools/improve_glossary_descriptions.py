...
def generate_concise_description(title: str, entry_type: str, reference_desc: Optional[str] = None) -> str:
    ...
    if entry_type == 'Definition':
        ...
        elif 'lsi' in title_lower or 'logarithmic sobolev' in title_lower:
        ...
        elif 'lipschitz' in title_lower:
            return f"Lipschitz continuity of {clean_title.replace('Lipschitz', '').strip()[:40]}"[:80]
        else:
    ...
    elif entry_type in ('Axiom', 'Assumption'):
        return f"Assumes {clean_title.replace('Axiom of', '').replace('Axiom', '').strip()[:60]}"[:80]
