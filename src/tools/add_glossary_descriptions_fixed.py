...
def generate_smart_description(title: str, entry_type: str) -> str:
    ...
    if entry_type == 'Definition':
        ...
        if 'bound' in title_lower:
            return f"Technical bound for analysis"
        elif 'decomposition' in title_lower:
            return f"Decomposes quantity into components"
        elif 'continuity' in title_lower or 'lipschitz' in title_lower:
            return f"Establishes regularity property"
        else:
        ...
        desc = f"Direct consequence of main result"
        return desc
        ...
        return f"Computational procedure for {clean_title.replace('Algorithm', '').strip().lower()}"

    elif entry_type == 'Observation':
        return f"Observed property of system"
