...
class FitnessPotentialCalculation(Scene):
    ...
    def raw_measurements(self):
        ...
        reward_content = VGroup(
            ...
            MathTex(r"r_i = R(x_i) - c_v ||v_i||^2 - \varphi_{\text{barrier}}(x_i)", 
        ...
        diversity_content = VGroup(
            ...
            MathTex(r"d_i = d_{\text{alg}}(i, \text{companion}_i)", 
                   font_size=18, color=BLUE),
            Text("Distance to paired companion", font_size=16, color=WHITE),
        ).arrange(DOWN, buff=0.25, aligned_edge=LEFT)
        
        diversity_box = Rectangle(width=4.5, height=2.5, color=BLUE, stroke_width=2, fill_opacity=0.05)
    ...
    def z_score_concept(self):
        ...
        # Large, clear formula
        formula = MathTex(
            r"z_i = \frac{\text{value}_i - \mu}{\sigma'}",
            font_size=56
        )
    ...
    def multiplicative_concept(self):
        ...
        main_formula = MathTex(
            r"V_i = (d'_i)^\beta \cdot (r'_i)^\alpha",
            font_size=64,
            color=YELLOW
        )
        main_formula.move_to(ORIGIN).shift(DOWN * 0.5)
        
        box = SurroundingRectangle(main_formula, color=YELLOW, buff=0.5, corner_radius=0.3, stroke_width=4)
    ...
    def complete_example(self):
        ...
        final_box = SurroundingRectangle(steps[-1], color=GREEN, buff=0.3, corner_radius=0.2, stroke_width=3)
