...
class LangevinSingle(Scene):
    def construct(self):
        ...
        legend_items = VGroup(
            VGroup(Dot(color=YELLOW, radius=0.1), Text("Particle", font_size=20)).arrange(RIGHT, buff=0.2),
            VGroup(Arrow(ORIGIN, RIGHT*0.3, color=RED, stroke_width=3), Text("Velocity", font_size=20)).arrange(RIGHT, buff=0.2),
            VGroup(Line(ORIGIN, RIGHT*0.3, color=BLUE, stroke_width=3), Text("Trajectory", font_size=20)).arrange(RIGHT, buff=0.2),
    ...
    def create_heatmap(self, data):
        ...
        # Plot heatmap
        im = ax.imshow(data, cmap='RdYlBu_r', origin='lower', extent=[-3, 3, -3, 3])
        ax.axis('off')
