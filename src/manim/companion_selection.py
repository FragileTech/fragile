...
class CompanionSelection(Scene):
    ...
    def setup_problem(self):
        ...
        # Question
        question = Text("Which companion should the parent choose?", font_size=32, color=YELLOW)
    ...
    def viscous_forces(self):
        ...
        companions_pos = [np.array([2, 1.5, 0]), np.array([-1.5, 2, 0]), np.array([1, -2, 0])]
        companions = VGroup()
        force_arrows = VGroup()
        force_labels = VGroup()
    ...
    def momentum_phases(self):
        ...
        self.play(
            Rotate(phase_vector, angle=PI, about_point=plane.n2p(0)), run_time=3, rate_func=linear
        )
        self.wait(1)
    ...
    def complex_amplitude(self):
        ...
        explanation = Text("Combine the viscous force with the phase:", font_size=28)
    ...
    def quantum_interference(self):
        ...
        for i in range(n_companions):
            ...
            label.next_to(
                amplitude_arrow.get_end(),
                direction=amplitude_end - companion.get_center(),
                buff=0.1,
            )

            self.play(FadeIn(companion), GrowArrow(amplitude_arrow), Write(label), run_time=0.5)

        self.wait(2)

        # Selection probability formula
        formula = MathTex("P(\\text{select } i) \\propto |c_i|^2", font_size=36, color=YELLOW)
        ...
        explanation_bg = BackgroundRectangle(explanation, color=BLACK, fill_opacity=0.8, buff=0.3)
