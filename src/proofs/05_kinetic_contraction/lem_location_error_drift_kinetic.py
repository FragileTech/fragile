...
from sympy import eye, Matrix, simplify, symbols, zeros
...
def test_drift_matrix_calculation():
    ...
    for d in (1, 3):
        ...
        M = Matrix.vstack(Matrix.hstack(Z, Id), Matrix.hstack(Z, -gamma * Id))

        Q = Matrix.vstack(
            Matrix.hstack(Id, (b / 2) * Id), Matrix.hstack((b / 2) * Id, lambda_v * Id)
        )
        ...
        assert (
            D[0:d, d : 2 * d] == (1 - (b * gamma) / 2) * Id
        ), f"Top-right block incorrect for d={d}."
        assert (
            D[d : 2 * d, 0:d] == (1 - (b * gamma) / 2) * Id
        ), f"Bottom-left block incorrect for d={d}."
        assert (
            D[d : 2 * d, d : 2 * d] == (b - 2 * gamma * lambda_v) * Id
        ), f"Bottom-right block incorrect for d={d}."
