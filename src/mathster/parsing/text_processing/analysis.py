...
def classify_label(label: str) -> str:
    ...
    if label.startswith('def-'):
        ...
        return 'proof'
    elif label.startswith('axiom-') or label.startswith('ax-') or label.startswith('def-axiom-'):
        return 'axiom'
...
def compare_extraction_with_source(
    ...
    for entity_type in ['definitions', 'theorems', 'proofs', 'axioms', 'parameters', 'remarks', 'assumptions']:
    ...
    # Add summary
    report_lines.insert(0, f"Total: Extracted {total_extracted}, Found {total_found}, Missed {total_missed}")
