...
def find_directive_lines(
    markdown_content: str, label: str, directive_type: str | None = None
) -> tuple[int, int] | None:
    """
    Find the line range for a Jupyter Book directive by its label.

    Jupyter Book directives have three common formats:

    Format 1 (label on same line):
    :::{prf:theorem} label-name
    content
    :::

    Format 2 (label after opening - STANDARD):
    :::{prf:theorem} Title Text
    :label: label-name
    content
    :::

    Format 3 (label before opening - PROOFS):
    ##### Heading
    :label: proof-name
    :::{prf:proof}
    content
    ...
    This function handles all formats and returns the full directive block line range.

    Args:
        markdown_content: The full markdown file content as a string
        label: The label to search for (e.g., "thm-keystone")
    ...
    Examples:
        ...
        ... :::
        ... '''
        >>> find_directive_lines(content, "proof-main")
        (3, 5)
    ...
    # STRATEGY 1: Look for :label: label on its own line
    # This is the standard Jupyter Book format
    label_pattern = rf"^:label:\s+{re.escape(label)}\s*$"

    for i, line in enumerate(lines):
        if re.match(label_pattern, line.strip()):
            # Found label line
            # Build directive pattern
            if directive_type:
                directive_pattern = rf"^:::\s*\{{prf:{directive_type}\}}"
            else:
                directive_pattern = r"^:::\s*\{prf:\w+\}"

            # STRATEGY 1A: Search backwards for opening directive
            # (Format 2: directive before label)
            for j in range(i - 1, max(-1, i - 10), -1):  # Search up to 10 lines back
                directive_line = lines[j].strip()

                if re.match(directive_pattern, directive_line):
            ...
            # STRATEGY 1B: Search forwards for opening directive
            # (Format 3: directive after label - common for mathster)
            for j in range(i + 1, min(len(lines), i + 10)):  # Search up to 10 lines forward
                directive_line = lines[j].strip()
    ...
    # STRATEGY 2: Look for label on same line as directive (fallback)
    # Format 1: :::{prf:theorem} label-name
    if directive_type:
        # Specific directive type (e.g., prf:theorem)
        pattern = rf"^:::\s*\{{prf:{directive_type}\}}\s+{re.escape(label)}"
    else:
        # Any directive type
        pattern = rf"^:::\s*\{{prf:\w+\}}\s+{re.escape(label)}"
