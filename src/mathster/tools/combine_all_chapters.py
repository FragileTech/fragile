...
Usage:
    python -m fragile.mathster.combine_all_chapters
    python -m fragile.mathster.combine_all_chapters --output combined_registry
"""
    ...
    try:
        if label:
            ...
            return SourceLocationBuilder.from_jupyter_directive(
                document_id=document_id, file_path=file_path, label=label
            )
...
class ChapterCombiner:
    ...
    def save_registry(self, output_dir: Path):
        ...
        print("\nTo regenerate:")
        print("  python -m fragile.mathster.combine_all_chapters --output combined_registry")
...
def main():
    ...
    # Setup paths
    # Script is in src/fragile/mathster/tools/, go up 4 levels to project root
