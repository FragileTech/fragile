...
from mathster.tools.line_finder import (
    find_directive_lines,
    find_text_in_markdown,
    get_file_line_count,
    validate_line_range,
)
from mathster.utils.source_helpers import SourceLocationBuilder
...
# Directive label pattern (matches thm-keystone, def-walker, etc.)
DIRECTIVE_LABEL_PATTERN = re.compile(r"^[a-z][a-z0-9-]*$")
...
def extract_directive_label_from_entity(entity_data: dict[str, Any]) -> str | None:
    ...
    1. label - primary field for directive labels (e.g., "thm-keystone")
    2. label_text - fallback if it matches directive pattern
    3. temp_id - legacy field

    Args:
        entity_data: Dictionary containing the raw entity data

    Returns:
        Directive label if extractable and valid, None otherwise

    Examples:
        >>> entity = {"label": "thm-keystone"}
        >>> extract_directive_label_from_entity(entity)
        "thm-keystone"

        >>> entity = {"label_text": "Mean-Square Continuity"}
        >>> extract_directive_label_from_entity(entity)
        None  # Not a valid directive label

    Maps to Lean:
        def extract_directive_label_from_entity (entity : Json) : Option String
    """
    # Check for label field first (primary source)
    if "label" in entity_data:
        label = entity_data["label"]
        if isinstance(label, str) and DIRECTIVE_LABEL_PATTERN.match(label):
            return label

    # Check for label_text (fallback, but must match pattern)
    if "label_text" in entity_data:
        label_text = entity_data["label_text"]
        if isinstance(label_text, str) and DIRECTIVE_LABEL_PATTERN.match(label_text):
            return label_text

    # Check temp_id for hints (legacy)
    if "temp_id" in entity_data:
        ...
        if isinstance(temp_id, str) and not temp_id.startswith("raw-"):
            if DIRECTIVE_LABEL_PATTERN.match(temp_id):
                return temp_id
...
def find_entity_location(
    ...
    label = extract_directive_label_from_entity(entity_data)
    if label:
