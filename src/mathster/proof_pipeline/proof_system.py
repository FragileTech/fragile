...
3. Recursive expansion: Complex steps become sub-mathster, simple steps are LLM-derived
4. Dataflow graph: Create explicit dependency graph for proof validation
5. Composability: Proofs reference other mathster as black-box transformations
...
class ProofBox(BaseModel):
    ...
    sub_proofs: dict[str, ProofBox] = Field(
        default_factory=dict, description="Nested sub-mathster (for complex steps)"
    )
...
class ProofEngine:
    ...
    4. Managing recursive sub-mathster
    """
