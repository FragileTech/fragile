...
# ---------- Leaf models used by the extracted representation ----------
...
class UnifiedLemma(BaseModel):
    """
    Unified lemma merging one directive item (lemma_raw.json)
    with one extracted item (lemma.json).
    """

    # Core identity
    label: str
    title: str | None = None
    type: str | None = Field(default="lemma")

    # Readable text / content
    nl_statement: str | None = None
    content_markdown: str | None = Field(
        default=None,
        description="Cleaned plaintext/markdown from directive content (line numbers stripped).",
    )
    ...
    @classmethod
    def from_instances(cls, directive: dict[str, Any], extracted: dict[str, Any]) -> UnifiedLemma:
        """
        Build a UnifiedLemma from:
          - directive: a dict for ONE directive-level lemma OR a container with 'items'.
          - extracted: a dict for ONE extracted lemma object.

        Strategy:
          * Match by 'label' when possible.
          * Pull raw text + positions from the directive item (lemma_raw.json).  :contentReference[oaicite:2]{index=2}
          * Pull structured fields (nl_statement, equations, hypotheses, variables, proof, tags) from the extracted item (lemma.json).  :contentReference[oaicite:3]{index=3}
          * Clean 'content' by stripping leading line numbers like '575:' per line.
        """

        # If a full directive container was passed, try to locate the matching item.
        ...
        # Provenance and registry context
...
def _build_unified_lemmas(
    ...
    for extracted in extracted_items:
        ...
        if not isinstance(label, str) or not label:
            logger.warning("Skipping extracted lemma without a label: %s", extracted)
            continue
        ...
        try:
            unified_obj = UnifiedLemma.from_instances(directive_argument, extracted)
        except Exception as exc:  # pragma: no cover - defensive logging
            logger.warning("Failed to merge lemma %s: %s", label, exc)
            continue
    ...
    if missing_labels:
        logger.warning(
            "Skipped %s extracted lemma(s) without directive matches: %s",
            len(missing_labels),
            ", ".join(sorted(set(missing_labels))),
        )
    if unmatched_directives:
        logger.info(
            "Directive lemmas without extraction counterparts: %s",
...
def _select_existing_file(candidates: list[Path]) -> Path:
    ...
    raise FileNotFoundError(
        "None of the expected lemma registry files were found: "
        + ", ".join(str(path) for path in candidates),
    )


def preprocess_document_lemmas(
    document: str | Path,
    *,
    ...
    directive_candidates = [
        registry_dir / "directives" / "lemma.json",
        registry_dir / "directives" / "lemma_raw.json",
    ]
    directives_path = select_existing_file(directive_candidates)

    extract_dir = resolve_extract_directory(registry_dir)
    extracted_candidates = [
        extract_dir / "lemma.json",
        extract_dir / "lemma_extracted.json",
    ]
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Merge directives/lemma*.json and extract/lemma*.json into "
            "registry/preprocess/lemma.json for a single document."
        ),
    )
    parser.add_argument(
        "document",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        help="Optional custom destination for the generated lemma.json file.",
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        output_path = preprocess_document_lemmas(args.document, output_path=args.output)
