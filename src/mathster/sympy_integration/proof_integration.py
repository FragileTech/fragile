...
- Graceful fallback: When SymPy can't validate, allow LLM mathster
...
class DirectDerivationWithSymPy(DirectDerivation):
    ...
    sympy_step: SymPyProofStep | None = Field(
        None,
...
def validate_proof_step_with_sympy(
    step: ProofStep,
    validator: SymPyValidator | None = None,
) -> ValidationResult:
    """
...
def validate_proof_chain_with_sympy(
    ...
    for i, step in enumerate(steps):
        ...
        if not result.is_valid:
            from mathster.sympy_integration.validation import ValidationIssue
...
def validate_proof_box_with_sympy(
    ...
    Args:
        ...
        recursive: If True, recursively validate sub-mathster
    ...
    # Recursively validate sub-mathster
...
def create_sympy_proof_step(
    ...
    result = validator.validate_chain(list(output_statements))
...
def add_sympy_validation_to_step(
    ...
    if step.step_type.value != "direct_derivation" or step.derivation is None:
        msg = "Can only add SymPy validation to direct derivations"
        raise ValueError(msg)

    if not isinstance(step.derivation, DirectDerivation):
        msg = "Derivation must be DirectDerivation"
        raise ValueError(msg)
