"""Fractal-set IG/CST area-law computation and plotting utilities.

Extracted from ``gravity.py`` to keep the UI builder separate from
pure computation / plotting logic.
"""
...
def compute_fractal_set_measurements(
    ...
    companions_dist = to_numpy(history.companions_distance).astype(np.int64, copy=False)
    companions_fit = to_numpy(history.companions_clone).astype(np.int64, copy=False)
    will_clone = to_numpy(history.will_clone).astype(bool, copy=False)
    x_pre = to_numpy(history.x_before_clone)
    volume_series = (
        to_numpy(history.riemannian_volume_weights)
        if getattr(history, "riemannian_volume_weights", None) is not None
        else None
    )
    metric_series = (
        to_numpy(history.diffusion_tensors_full)
...
def build_fractal_set_scatter_plot(
...
def format_fractal_set_summary(
    points_df: pd.DataFrame,
    regression_df: pd.DataFrame,
...
def build_fractal_set_baseline_comparison(regression_df: pd.DataFrame) -> pd.DataFrame:
...
def resolve_electroweak_geodesic_matrices(
...
def build_geodesic_distance_distribution_by_frame(
...
def build_geodesic_distribution_plot(
