...
class DelaunayGeometryData:
    ...
    neighbor_matrix: Tensor  # [N, K]
    neighbor_mask: Tensor  # [N, K]
    neighbor_counts: Tensor  # [N]
    ...
    # Hessian / metric
    hessian_diag: Tensor | None  # [N, d]
    hessian_full: Tensor | None  # [N, d, d]
    hessian_valid_mask: Tensor  # [N]
    metric_tensors: Tensor  # [N, d, d]
    metric_det: Tensor  # [N]
    riemannian_volume_weights: Tensor  # [N] = sqrt(det g)
    diffusion_tensors: Tensor  # [N, d, d]

    # Weights on edges
    edge_weights: dict[str, Tensor]

    # Ricci proxy (scalar curvature)
    ricci_proxy: Tensor  # [N]
    ricci_proxy_full: Tensor | None = None  # [N]
    ricci_tensor_full: Tensor | None = None  # [N, d, d] anisotropic proxy
...
def compute_metric_from_hessian(
    ...
    if hessian_full is not None:
        ...
        det, volume = compute_det_and_volume(metric)
...
def compute_det_and_volume(metric_tensors: Tensor, eps: float = 1e-12) -> tuple[Tensor, Tensor]:
...
def compute_diffusion_tensor(metric_tensors: Tensor, min_eig: float = 1e-6) -> Tensor:
...
def compute_delaunay_data(
    ...
    if spatial_dims is not None:
        pos = positions[:, :spatial_dims]
    else:
        pos = positions
    fit = fitness_values

    if pos.shape[0] == 0:
        msg = "No walkers for Delaunay computation."
        raise ValueError(msg)

    positions_np = pos.detach().cpu().numpy()
    edges_np = build_delaunay_edges(positions_np)
    ...
    csr = build_csr_from_coo(edge_index, n_nodes=pos.shape[0])
    ...
    hessian_diag = None
    hessian_full = None
    hessian_valid = torch.ones(pos.shape[0], dtype=torch.bool, device=device)
    ...
    if metric_mode == "covariance":
        ...
        metric_det, volume_weights = compute_det_and_volume(metric_tensors)
    elif metric_mode == "hessian":
        if hessian_mode == "none":
            msg = "hessian_mode must not be 'none' when metric_mode='hessian'."
            raise ValueError(msg)
        metric_tensors, metric_det, volume_weights = compute_metric_from_hessian(
    ...
    # Edge distances
    if edge_index.numel() == 0:
        edge_distances = torch.empty(0, device=device, dtype=pos.dtype)
    else:
        src, dst = edge_index
        edge_distances = torch.norm(pos[dst] - pos[src], dim=1)

    edge_geodesic = compute_geodesic_distances(pos, edge_index, metric_tensors)

    diffusion_tensors = compute_diffusion_tensor(metric_tensors, min_eig=min_eig or 1e-6)
    ...
    cell_volumes = torch.ones(pos.shape[0], device=device, dtype=pos.dtype)
    ...
    if ricci_weights is None:
        if edge_index.numel() == 0:
            ricci_weights = torch.empty(0, device=device, dtype=pos.dtype)
        else:
            raw = 1.0 / (edge_geodesic + 1e-8)
            src = edge_index[0]
            sums = torch.zeros(pos.shape[0], device=device, dtype=raw.dtype)
