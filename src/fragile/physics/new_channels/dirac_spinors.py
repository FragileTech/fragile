...
def vector_to_weyl_spinor(
    ...
    # Flatten to 1-D batch for safe boolean indexing, then reshape back
    w_flat = w.reshape(-1, 3).float()

    w1 = w_flat[:, 0]
    w2 = w_flat[:, 1]
    w3 = w_flat[:, 2]
    r = torch.linalg.vector_norm(w_flat, dim=-1)  # [B]
    ...
    B = w_flat.shape[0]
    # Allocate spinor
    spinor = torch.zeros(B, 2, device=w.device, dtype=torch.complex64)
    ...
    return spinor.reshape(*orig_shape, 2), valid.reshape(orig_shape)
...
def compute_dirac_operators_from_spinors(
    ...
    op_tensor_0k = torch.where(valid_mask, op_tensor_0k, zero)
