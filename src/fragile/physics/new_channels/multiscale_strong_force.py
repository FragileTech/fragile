...
from fragile.physics.qft_utils.statistics import correlators_with_statistics, ensure_statistics
    ...
    *,
    ...
    for s_idx in range(n_scales):
        ...
        if "tensor_companion" in out or "tensor_traceless_companion" in out:
            ...
            # The contracted correlator is the component-summed autocorrelation
            # of the (cos, sin) operator components, not of the norm series
            # above (which is only kept as a display diagnostic). Recover the
            # origin statistics from the actual operator so the channel can be
            # fitted with measured uncertainties.
            tensor_operator = torch.cat(
                [
                    tensor_out.momentum_operator_cos_series[0],
                    tensor_out.momentum_operator_sin_series[0],
                ],
                dim=0,
            ).t()  # [T, 2 * n_components]
            # The correlator was formed over frames with at least one valid pair.
            tensor_valid_frames = tensor_out.component_counts_per_frame > 0
            if tensor_valid_frames.shape[0] == tensor_operator.shape[0]:
                tensor_operator = tensor_operator[tensor_valid_frames]
            tensor_correlator = ensure_statistics(
                tensor_out.momentum_contracted_correlator[0], tensor_operator
            )
...
def compute_multiscale_strong_force_channels(
    ...
    *,
    ...
    # Exact origin statistics for every (channel, scale) series. The float32 FFT
    # estimator cannot always be matched by ``ensure_statistics`` within
    # tolerance, which would silently leave every channel without a measured
    # mass uncertainty and make the best-scale error filter reject all of them.
    corr_with_stats = correlators_with_statistics(
        stack.reshape(c_count * s_count, t_len),
        max_lag=int(correlator_cfg.max_lag),
        connected=bool(correlator_cfg.use_connected),
        dtype=stack.dtype,
    )
        ...
        if companion_override_results.get(channel):
            ...
            for s_idx in range(s_count):
                ...
                result = _build_result_from_precomputed_correlator(
                    ...
                    correlator=corr_with_stats[c_idx * s_count + s_idx],
                    dt=dt,
                    config=correlator_cfg,
                    n_samples=t_len,
