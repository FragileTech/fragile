...
def main() -> None:
    ...
    if analysis_cfg.build_fractal_set:
        ...
        curvature_hessian = compute_ricci_from_fractal_set_hessian(fractal_set, timestep=timestep)
        cheeger = None
    ...
    d_plot = plot_correlation_decay(
        d_bins,
        d_corr,
        d_counts,
        d_fit,
        "Diversity Correlation Decay",
        plots_dir / f"{analysis_id}_d_prime_corr.html",
    )
    r_plot = plot_correlation_decay(
        r_bins,
        r_corr,
        r_counts,
        r_fit,
        "Reward Correlation Decay",
        plots_dir / f"{analysis_id}_r_prime_corr.html",
    )
    lyap_plot = plot_lyapunov(
        lyapunov["time"].cpu().numpy(),
        lyapunov["V_total"].cpu().numpy(),
        lyapunov["V_var_x"].cpu().numpy(),
        lyapunov["V_var_v"].cpu().numpy(),
        plots_dir / f"{analysis_id}_lyapunov.html",
    )
    phase_plot = plot_phase_histograms(
        u1_phases,
        su2_phases,
        alive,
        plots_dir / f"{analysis_id}_phase_hist.html",
    )
    wilson_hist_plot = None
    wilson_time_plot = None
    if wilson_metrics is not None:
        wilson_hist_plot = plot_wilson_histogram(
            wilson_metrics["wilson_values"],
            "Wilson Loop Distribution",
            plots_dir / f"{analysis_id}_wilson_hist.html",
        )
    if analysis_cfg.build_fractal_set and wilson_timeseries is not None:
        wilson_time_plot = plot_wilson_timeseries(
            wilson_timeseries["time_index"],
            wilson_timeseries["action_mean"],
            plots_dir / f"{analysis_id}_wilson_time.html",
        )
    ...
    if analysis_cfg.use_local_fields and local_correlations:
        ...
        for field_name, corr_data in local_correlations.items():
            ...
            plot_path = plot_correlation_decay(
                corr_data["bins"],
                corr_data["correlation"],
                corr_data["counts"],
                corr_data["fit"],
                f"{title} Correlation{connected_suffix}",
                plots_dir / f"{analysis_id}_{field_name}_corr{connected_suffix}.html",
            )
            if plot_path:
