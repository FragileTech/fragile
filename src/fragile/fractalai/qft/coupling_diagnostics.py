...
def _safe_gather_pairs_2d(values: Tensor, indices: Tensor) -> tuple[Tensor, Tensor]:
    ...
    idx = indices.clamp(min=0, max=max(n - 1, 0))
    gathered = torch.gather(values, 1, idx.reshape(t, -1)).reshape_as(indices)
...
def _safe_gather_pairs_3d(values: Tensor, indices: Tensor) -> tuple[Tensor, Tensor]:
    ...
    gathered = torch.gather(
        values,
        1,
        idx.reshape(t, -1).unsqueeze(-1).expand(-1, -1, c),
    ).reshape(t, indices.shape[1], indices.shape[2], c)
    return gathered, in_range
    ...
    *,
    ...
    try:
        scales = select_interesting_scales_from_history(
            ...
            assume_all_alive=bool(cfg.kernel_assume_all_alive),
    ...
    node_valid = (alive[t_snapshot] & color_valid[t_snapshot]).to(torch.bool)
...
def compute_coupling_diagnostics(
    ...
    color_j, pair_in_range = _safe_gather_pairs_3d(color, pair_indices)
    ...
    walker_valid = alive & color_valid & row_has_pairs
