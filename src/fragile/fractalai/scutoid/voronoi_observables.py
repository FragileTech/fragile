...
def _select_alive_volume_weights(
    volume_weights: np.ndarray | torch.Tensor | None,
    voronoi_data: dict[str, Any],
) -> np.ndarray | None:
    if volume_weights is None:
        return None
    if torch.is_tensor(volume_weights):
        volume_weights = volume_weights.detach().cpu().numpy()
    volume_weights = np.asarray(volume_weights).reshape(-1)

    volumes = voronoi_data.get("volumes")
    n_alive = len(volumes) if volumes is not None else 0
    if n_alive == 0:
        return np.array([])

    if volume_weights.size == n_alive:
        return volume_weights

    alive_indices = voronoi_data.get("alive_indices")
    if alive_indices is None:
        return volume_weights[:n_alive]

    alive_idx = np.asarray(alive_indices).reshape(-1)
    if alive_idx.size == 0:
        return np.zeros(n_alive, dtype=float)
    if volume_weights.size <= int(alive_idx.max()):
        return volume_weights[:n_alive]
    return volume_weights[alive_idx]


def _interior_mask(
    voronoi_data: dict[str, Any],
    n_cells: int,
    include_boundary_adjacent: bool = True,
) -> np.ndarray | None:
    classification = voronoi_data.get("classification")
    if classification is None:
        return None
    tiers = classification.get("tier")
    if tiers is None:
        return None
    if torch.is_tensor(tiers):
        tiers = tiers.detach().cpu().numpy()
    tiers = np.asarray(tiers).reshape(-1)
    if tiers.size != n_cells:
        return None
    if include_boundary_adjacent:
        return tiers >= 2
    return tiers >= 1
...
def compute_riemannian_volume_weights(
    voronoi_data: dict[str, Any],
    sigma: torch.Tensor,
    c2: float | torch.Tensor | None = None,
    full_size: int | None = None,
    eps: float = 1e-12,
) -> torch.Tensor | None:
    """Compute Riemannian volume weights from Voronoi volumes and diffusion tensor.

    The approximation uses the emergent metric determinant:
        sqrt(det g) ≈ c2^d / det(sigma)
    ...
    Args:
        ...
        sigma: Diffusion tensor (diag [N, d] or full [N, d, d]).
        c2: Isotropic diffusion scale (defaults to 1.0 if None).
        full_size: If provided, expand weights to length full_size (fill missing with 0).
        eps: Numerical clamp for stability.

    Returns:
        Tensor of Riemannian volume weights, or None if sigma is None.
    """
    if sigma is None:
        return None
    ...
    if n_alive == 0:
        if full_size is not None:
            return torch.zeros(full_size, device=sigma.device, dtype=sigma.dtype)
        return torch.zeros(0, device=sigma.device, dtype=sigma.dtype)

    if c2 is None:
        c2_val = torch.tensor(1.0, device=sigma.device, dtype=sigma.dtype)
    elif torch.is_tensor(c2):
        c2_val = c2.to(device=sigma.device, dtype=sigma.dtype)
    else:
        c2_val = torch.tensor(float(c2), device=sigma.device, dtype=sigma.dtype)

    d = sigma.shape[-1]

    if sigma.shape[0] == n_alive:
        sigma_alive = sigma
    else:
        alive_indices = voronoi_data.get("alive_indices")
        if alive_indices is None:
            sigma_alive = sigma[:n_alive]
        else:
            alive_idx = torch.as_tensor(alive_indices, device=sigma.device, dtype=torch.long)
            alive_idx = alive_idx[alive_idx < sigma.shape[0]]
            if alive_idx.numel() == 0:
                sigma_alive = sigma[:n_alive]
            else:
                sigma_alive = sigma.index_select(0, alive_idx)

    if sigma_alive.dim() == 2:
        sigma_safe = torch.clamp(sigma_alive, min=eps)
        log_det_sigma = torch.log(sigma_safe).sum(dim=-1)
    else:
        sigma_sym = 0.5 * (sigma_alive + sigma_alive.transpose(-1, -2))
        sign, logdet = torch.linalg.slogdet(sigma_sym)
        if (sign <= 0).any():
            det = torch.det(sigma_sym)
    ...
    interior_mask = _interior_mask(voronoi_data, n_alive, include_boundary_adjacent=True)
    if interior_mask is not None:
        mask_tensor = torch.as_tensor(
            interior_mask, device=weights_alive.device, dtype=weights_alive.dtype
        )
        weights_alive = weights_alive * mask_tensor

    if full_size is None and sigma.shape[0] != n_alive:
        full_size = sigma.shape[0]

    if full_size is None:
        return weights_alive

    weights_full = torch.zeros(full_size, device=sigma.device, dtype=sigma.dtype)
    alive_indices = voronoi_data.get("alive_indices")
    if alive_indices is None:
        n_copy = min(n_alive, full_size)
        weights_full[:n_copy] = weights_alive[:n_copy]
    else:
        alive_idx = torch.as_tensor(alive_indices, device=sigma.device, dtype=torch.long)
        alive_idx = alive_idx[alive_idx < full_size]
        n_copy = min(alive_idx.shape[0], weights_alive.shape[0])
        if n_copy > 0:
            weights_full[alive_idx[:n_copy]] = weights_alive[:n_copy]
...
def compute_geometric_weights(
    ...
    normalize: bool = True,
    volume_weights: np.ndarray | torch.Tensor | None = None,
) -> dict[str, Any]:
    """
    ...
    Args:
        ...
        volume_weights: Optional Riemannian volume weights (replaces Euclidean volumes)
    ...
    if volume_weights_alive is not None and len(volume_weights_alive) == n:
        volumes_eff = volume_weights_alive

    node_weights = torch.from_numpy(np.asarray(volumes_eff)).float()
    ...
    if weight_mode == "facet_area":
        ...
        if normalize and np.asarray(volumes_eff).size > 0:
            mean_vol = np.asarray(volumes_eff).mean()
            if mean_vol > 0:
                node_weights = torch.from_numpy(np.asarray(volumes_eff) / mean_vol).float()
...
def compute_curvature_proxies(
    ...
    Args:
        ...
        volume_weights: Optional Riemannian volume weights (overrides Euclidean volumes)
    ...
    valid_mask = None
    if interior_mask is not None:
        valid_mask = interior_mask & np.isfinite(volumes_eff)
    else:
        valid_mask = np.isfinite(volumes_eff)

    mean_vol = (
        volumes_eff[valid_mask].mean() if valid_mask is not None and valid_mask.any() else 1.0
    )
    normalized_volumes = np.ones_like(volumes_eff)
    if mean_vol > 0 and valid_mask is not None and valid_mask.any():
        normalized_volumes[valid_mask] = volumes_eff[valid_mask] / mean_vol
    volume_variance = (
        float(np.var(normalized_volumes[valid_mask]))
