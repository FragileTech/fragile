...
def remove_next_prefix(name: str, next_prefix: str) -> str:
    if name.startswith(next_prefix):
        return name[len(next_prefix) :]
    return name


def to_node_id(x) -> int:
    return x
...
class NetworkxTree(BaseTree):
    ...
    def __init__(
        ...
        """
        super().__init__(root_id=to_node_id(root_id))
        self.next_prefix = next_prefix
        self.names = names if names is not None else []
        self.data: nx.DiGraph = nx.DiGraph()
        self.data.add_node(DEFAULT_ROOT_ID)
        self._node_count = 0
        self.leafs = set()
        self.frozen_node_ids = set()
        # Node and edge names are parsed with no prefix
        self.prune = prune
        self.node_names = [
            remove_next_prefix(name, next_prefix)
            for name in self.names
            if remove_next_prefix(name, next_prefix) in node_names
        ]
        self.edge_names = [
            remove_next_prefix(name, next_prefix)
            for name in self.names
            if remove_next_prefix(name, next_prefix) in edge_names
        ]
        # The pruner sentinel behaves as the root node for the pruning process. This allows
        # to avoid iterating recursively until the root node is reached.
    ...
    def reset(self, root_id: NodeId = None, module: StateProvider | None = None) -> None:
        ...
        if module is not None:
            node_data = self._extract_data_from_states(index=0, only_node_data=True, module=module)
        else:
            node_data = {}
        self.reset_graph(root_id=to_node_id(root_id), node_data=node_data, epoch=-1)

    def node_is_redundant(self, node_data, parent_id: NodeId, names: tuple[str, ...]) -> bool:
        """Return True if the node is present in the graph and it contains a terminal state."""
        child_is_terminal = any(n in node_data and node_data[n].squeeze() for n in names)
        if parent_id not in self.data.nodes:
            return child_is_terminal
        parent = self.data.nodes[parent_id]
        for name in names:
            if name in parent and parent[name].squeeze():  # Parent is terminal
                return child_is_terminal
        return False
    ...
    def update(
        ...
        freeze_parents: list[NodeId] | None = None,
        n_iter: int = -1,
        terminal_names: tuple[str, ...] = ("terminals", "oobs"),
    ):
        """Update the history of the tree adding the necessary data to recreate \
        the trajectories sampled by the :class:`Swarm`.

        Args:
            node_ids: List of states hashes representing the node_ids of \
                the current states. They are assumed to be leafs of the tree.
            parent_ids: List of states hashes representing the parent nodes of \
                the current states.
        ...
        for i, (leaf, parent) in enumerate(zip(leaf_ids, parent_ids)):
            node_data, edge_data = self._extract_data_from_states(index=i, module=module)
            if not self.node_is_redundant(node_data, parent, terminal_names):
                self.append_leaf(
                    leaf_id=leaf,
                    parent_id=parent,
                    node_data=node_data,
    ...
    def append_leaf(
        ...
        skip_nodes = {DEFAULT_ROOT_ID, DEFAULT_FIRST_NODE_ID}
        if leaf_id not in self.data.nodes:
            if parent_id not in self.data.nodes:
                msg = f"Parent {parent_id} of leaf {leaf_id} not in graph."
                raise ValueError(msg)
            if leaf_id not in skip_nodes and not all(k in node_data for k in self.node_names):
                missing = set(self.node_names) - set(node_data.keys())
                msg = f"Node data does not contain all the required keys. Missing {missing}"
                raise ValueError(msg)
            if leaf_id not in skip_nodes and not all(k in edge_data for k in self.edge_names):
                missing = set(self.edge_names) - set(edge_data.keys())
                msg = f"Edge data does not contain all the required keys. Missing {missing}"
                raise ValueError(msg)
    ...
    def prune_dead_branches(self, dead_leafs: set[NodeId], alive_leafs: set[NodeId]) -> None:
        ...
        Args:
            ...
            alive_leafs: Leaves of the branches that we can continue to expand.

        Returns:
            None

        """
        alive_nodes = alive_leafs | self.frozen_node_ids
    ...
    def get_parent(self, node_id: NodeId) -> NodeId:
        ...
        try:
            return next(iter(self.data.in_edges(to_node_id(node_id))))[0]
        except (KeyError, IndexError):
            msg = f"Node {node_id} does not have a parent in the graph"
            raise KeyError(msg)
    ...
    def get_branch(self, leaf_id: NodeId) -> list[NodeId]:
        ...
        leaf_id = to_node_id(leaf_id)
        nodes = [leaf_id]
        while leaf_id not in {self.root_id, DEFAULT_FIRST_NODE_ID}:
    ...
    def get_path_node_ids(self, leaf_id: NodeId, root_id: NodeId = None) -> np.ndarray:
        ...
        Args:
            leaf_id: Unique value that identifies the end of the path, usually a leaf of the tree.
            root_id: Node id of the start of the path, defaults to the root node of the tree.
        ...
        return numpy.array(nodes, dtype=HASH_DTYPE)
    ...
    def _update_prune_sentinel(self):
        """Move the prune sentinel forward to the first node with more than a single child.

        After some time evolving the parent, the branches originating from the root node
        will be pruned until one single branch remains alive. Once this happens, the
        prune sentinel will be moved to the first node with more than a single child.

        The new prune_sentinel node will behave as the root node for the pruning process.
        This avoids iterating recursively until the root node is reached.
        """
        children_edges = self.data.out_edges([self._prune_sentinel])
        while len(children_edges) == 1:
            self._prune_sentinel = next(iter(children_edges))[1]  # Update to child node
            children_edges = self.data.out_edges([self._prune_sentinel])
    ...
    def _extract_data_from_states(
        ...
        node_data = module.get(index=index, names=self.node_names, as_dict=True)
        node_data = module.copy(**node_data, as_dict=True) if node_data else {}
        if only_node_data:
            return node_data
        edge_data = module.get(index=index, names=self.edge_names, as_dict=True)
        edge_data = module.copy(**edge_data, as_dict=True) if edge_data else {}
...
class DataTree(NetworkxTree):
    ...
    def _extract_one_node_data(self, data: NodeData, names: NamesData) -> tuple:
        ...
        def get_item(data, name):
            ...
            all_keys = set(tuple(node_data.keys()) + tuple(edge_data.keys()))
            msg = f"{name} not found in data. Available keys {all_keys}"
            raise KeyError(msg)

        return tuple(get_item(data, name) for name in names)

    def _process_batch(self, batch_data: list[tuple]) -> tuple[Value, ...]:
        ...
        for name_ in names:
            ...
            if name not in self.names:
                msg = (
                    ...
                    f"self.names: {self.names} for element {name[len(self.next_prefix) :]}"
                )
                raise KeyError(msg)
        return names
    ...
    def path_data_generator(
        self,
        node_ids: tuple[NodeId] | (set[NodeId] | list[NodeId]),
        ...
        for i, node in enumerate(node_ids[:-1]):
            ...
            if next_node in skip_nodes or node in skip_nodes:
                continue
