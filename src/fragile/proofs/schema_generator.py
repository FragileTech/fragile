...
Usage:
    ...
    from fragile.proofs.schema_generator import generate_complete_schema
    schema = generate_complete_schema()

    # Generate via CLI
    python -m fragile.proofs.schema_generator
    python -m fragile.proofs.schema_generator --output custom_schema.json
    python -m fragile.proofs.schema_generator --include-examples
...
def discover_all_models() -> Dict[str, List[Type[BaseModel]]]:
    ...
    for category, module_names in submodules.items():
        for module_name in module_names:
            try:
                module = importlib.import_module(f"fragile.proofs.{category}.{module_name}")
                # Find all BaseModel subclasses in module
                for name, obj in inspect.getmembers(module, inspect.isclass):
                    if (
                        issubclass(obj, BaseModel)
                        and obj is not BaseModel
                        and obj.__module__.startswith("fragile.proofs")
                    ):
...
def generate_proof_schema(
    ...
    publishable mathematical proofs with full validation.
    ...
    schema = {
        ...
        "workflow": {
            ...
            "common_patterns": [
                ...
                "Constructive proofs: Build object step-by-step, verify properties",
...
Examples:
  ...
  python -m fragile.proofs.schema_generator --proof

  # Generate proof sketch schema only
  python -m fragile.proofs.schema_generator --sketch
