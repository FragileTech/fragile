...
from fragile.proofs.sympy_integration.expressions import AnySymExpr, AssumptionSet, SymExpr
...
class DualExpr(BaseModel):
    ...
    latex: str = Field(
        ..., min_length=1, description="LaTeX representation (authoritative for display)"
    )
    sympy: SymExpr | None = Field(
        None, description="SymPy representation (authoritative for validation)"
    )
    parse_status: Literal["ok", "unavailable", "opaque", "failed"] = Field(
        "ok", description="Status of SymPy representation"
    )
    parse_error: str | None = Field(None, description="Error message if parse_status='failed'")
...
class DualStatement(BaseModel):
    ...
    context: str | None = Field(None, description="Additional context (e.g., 'for all x > 0')")

    def can_validate(self) -> bool:
        """Check if this statement can be validated with SymPy."""
        return (
            self.lhs.can_validate()
            and self.rhs.can_validate()
...
class SymbolDeclaration(BaseModel):
    ...
    assumptions: AssumptionSet = Field(
        default_factory=AssumptionSet, description="Assumptions about this symbol"
    )
...
class PaperContext(SymPyContext):
    ...
    def merge_with_local(self, local_context: SymPyContext) -> SymPyContext:
        ...
        merged_global = (
            self.global_assumptions + self.local_assumptions + local_context.global_assumptions
        )
...
def parse_latex_to_sympy(
    latex: str, context: SymPyContext | None = None
) -> tuple[AnySymExpr | None, str | None]:
    """
    ...
    try:
        ...
        from fragile.proofs.sympy_integration_expressions import from_sympy

        pydantic_expr = from_sympy(sympy_expr)
        return pydantic_expr, None
    except Exception as e:
        return None, f"LaTeX parse error: {e!s}"
...
def create_dual_expr_from_latex(latex: str, context: SymPyContext | None = None) -> DualExpr:
    """
    ...
    if error and "SymPy not available" in error:
...
def create_symbol_context(symbols: list[tuple[str, str, AssumptionSet]]) -> SymPyContext:
    ...
    for name, latex_name, assumptions in symbols:
        declarations[name] = SymbolDeclaration(
            name=name, latex_name=latex_name, assumptions=assumptions
        )

    return SymPyContext(symbols=declarations)


def validate_dual_consistency(dual: DualExpr) -> tuple[bool, str | None]:
    """
    ...
    try:
        ...
        " ".join(dual.latex.split())
        " ".join(sympy_latex.split())
        ...
        return False, f"Consistency check failed: {e!s}"
