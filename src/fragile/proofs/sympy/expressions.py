...
class AssumptionSet(BaseModel):
    ...
    lipschitz: Optional[bool] = None
    measurable: Optional[bool] = None
    integrable: Optional[bool] = None
...
class FloatExpr(SymExpr):
    ...
    precision: Optional[int] = Field(None, ge=1, description="Precision in bits")
...
class IntegralExpr(SymExpr):
    ...
    limits: List[Tuple["AnySymExpr", Optional["AnySymExpr"], Optional["AnySymExpr"]]] = Field(
...
class PiecewiseExpr(SymExpr):
    ...
    otherwise: Optional["AnySymExpr"] = Field(None, description="Default case")
    ...
    if isinstance(expr, sympy.Integral):
        ...
        for limit in expr.limits:
            if len(limit) == 1:
                ...
                # Definite
                limits_list.append(
                    (from_sympy(limit[0]), from_sympy(limit[1]), from_sympy(limit[2]))
                )
