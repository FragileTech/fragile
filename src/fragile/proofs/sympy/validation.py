...
from typing import TYPE_CHECKING, Any, Callable, Dict, List, Literal, Optional, Protocol, Tuple

from pydantic import BaseModel, ConfigDict, Field

from fragile.proofs.sympy.dual_representation import DualExpr, DualStatement, SymPyContext
from fragile.proofs.sympy.expressions import AnySymExpr, AssumptionSet
...
class ValidationIssue(BaseModel):
    ...
    location: Optional[str] = Field(None, description="Where the issue occurred (e.g., 'step 3', 'lhs')")
...
class ValidationResult(BaseModel):
    ...
    @classmethod
    def valid(cls, details: Optional[Dict[str, str]] = None) -> "ValidationResult":
...
class Transformation(BaseModel):
    ...
    def validate(self) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult.uncertain(f"Validation error: {str(e)}")
...
def simplify_expr(expr: DualExpr) -> Transformation:
    ...
    try:
        ...
        if sympy_expr is None:
            return Transformation(transformation_type=TransformationType.SIMPLIFY, input_expr=expr, output_expr=expr)

        simplified = sp.simplify(sympy_expr)

        from fragile.proofs.sympy.dual_representation import create_dual_expr_from_sympy
        from fragile.proofs.sympy.expressions import from_sympy

        output = create_dual_expr_from_sympy(from_sympy(simplified))

        return Transformation(transformation_type=TransformationType.SIMPLIFY, input_expr=expr, output_expr=output)
    except Exception:
        return Transformation(transformation_type=TransformationType.SIMPLIFY, input_expr=expr, output_expr=expr)
...
def expand_expr(expr: DualExpr) -> Transformation:
    ...
    if not SYMPY_AVAILABLE or not expr.can_validate():
        return Transformation(transformation_type=TransformationType.EXPAND, input_expr=expr, output_expr=expr)
    ...
    try:
        ...
        return Transformation(transformation_type=TransformationType.EXPAND, input_expr=expr, output_expr=output)
    except Exception:
        return Transformation(transformation_type=TransformationType.EXPAND, input_expr=expr, output_expr=expr)
...
class PluginRegistry(BaseModel):
    ...
    plugins: List[Any] = Field(default_factory=list, description="Registered plugins (must match ValidationPlugin protocol)")
    ...
    def register(self, plugin: Any) -> None:
        ...
        if not hasattr(plugin, "can_validate") or not hasattr(plugin, "validate"):
            raise ValueError("Plugin must have can_validate() and validate() methods")
        self.plugins.append(plugin)

    def find_plugin(self, statement: DualStatement) -> Optional[Any]:
...
class SymPyValidator(BaseModel):
    ...
    model_config = ConfigDict(frozen=False, arbitrary_types_allowed=True)  # Mutable for plugin registration

    context: SymPyContext = Field(default_factory=SymPyContext, description="Symbol table and assumptions")
    plugin_registry: PluginRegistry = Field(default_factory=PluginRegistry, description="Domain-specific plugins")
    strict_mode: bool = Field(
        False, description="If True, require SymPy validation (no LLM fallback for uncertain cases)"
    )
        ...
        try:
            ...
            if statement.relation == "=":
                ...
                try:
                    ...
                    return ValidationResult.uncertain(f"SymPy cannot validate inequality: {statement.relation}")
                except Exception:
