...
The `fragile.proofs` module implements a **two-stage Extract-then-Enrich pipeline** for processing mathematical documents written in Jupyter Book MyST markdown format. The system transforms human-readable mathematical text into structured, validated, machine-readable entities suitable for:
...
The `fragile.proofs` package contains **44 Python files** organized as follows:
