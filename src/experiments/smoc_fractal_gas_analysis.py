...
def compute_effective_mass_series(
    ...
    return torch.log(ratio) / delta_t
...
def run_smoc_analysis(
    ...
    masses = {
        ch: results["channels"][ch].get("mass", float("inf"))
        for ch in config.channels
        if "error" not in results["channels"].get(ch, {})
    }

    if "pion" in masses and "rho" in masses:
        results["validation"] = {
            "pion_lighter_than_rho": masses["pion"] < masses["rho"],
            "mass_ratio_rho_pion": masses["rho"] / masses["pion"]
            if masses["pion"] > 0
            else float("inf"),
        }
...
def plot_smoc_results(
    ...
    for channel, data in channels.items():
        ...
        ax_corr.errorbar(
            t, corr, yerr=corr_err, fmt="o-", markersize=3, capsize=2, label=f"{channel}"
        )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="outputs/smoc_analysis", help="Output directory"
    )
    parser.add_argument(
        "--warmup-fraction", type=float, default=0.1, help="Fraction of steps to discard as warmup"
    )
    parser.add_argument(
        "--channels",
        type=str,
    ...
    history, _potential = run_simulation(potential_cfg, operator_cfg, run_cfg, show_progress=True)
    ...
    for channel, data in results["channels"].items():
        if "error" in data:
            ...
            print(
                f"{channel:<12} {data['mass']:>10.4f} {data['mass_error']:>10.4f} {data['n_valid_windows']:>10}"
            )
