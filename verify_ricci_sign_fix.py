...
def main():
    ...
    print(f"  (Expected zeros on first call - no previous volumes)")
    print()
    ...
    print(f"  → Swarm expanded (dV/dt > 0, θ > 0)")
    print(f"  → Expected: R = -θ < 0 (NEGATIVE curvature)")
    ...
    print(f"  → Swarm contracted (dV/dt < 0, θ < 0)")
    print(f"  → Expected: R = -θ > 0 (POSITIVE curvature)")
