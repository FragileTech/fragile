...
### 2.4 HWI Inequality for Discrete-Time Cloning

We now bound the entropy change under revival using the HWI inequality (Theorem 0.4).

**Lemma 2.4 (HWI Bound for Revival Entropy)**:

For measures $\mu$ close to $\pi_{\text{QSD}}$ (in the sense $D_{\text{KL}}(\mu \| \pi_{\text{QSD}}) = O(\epsilon)$ small):

$$
D_{\text{KL}}(\Psi_{\text{revival}}^*\mu \| \pi_{\text{QSD}}) \le D_{\text{KL}}(\mu \| \pi_{\text{QSD}}) + C_{\text{HWI}} W_2(\mu, \pi_{\text{QSD}}) + O(\tau^2)
$$

where:

$$
C_{\text{HWI}} = O(1/\sqrt{\kappa_{\text{conf}}})
$$

is the HWI coupling constant (depends on log-Sobolev constant of $\pi_{\text{QSD}}$).

**Proof**:

**Step 1 - HWI inequality application**:

From Theorem 0.4 (Otto-Villani HWI inequality):

$$
\sqrt{D_{\text{KL}}(\mu \| \pi_{\text{QSD}})} \le W_2(\mu, \pi_{\text{QSD}}) \sqrt{\mathcal{I}(\mu \| \pi_{\text{QSD}})/2}
$$

**Step 2 - Fisher information bound via LSI**:

By the log-Sobolev inequality for $\pi_{\text{QSD}}$ (which is log-concave with LSI constant $C_{\text{LSI}}^{\text{Gibbs}} = O(1/\kappa_{\text{conf}})$):

$$
D_{\text{KL}}(\mu \| \pi_{\text{QSD}}) \le C_{\text{LSI}}^{\text{Gibbs}} \mathcal{I}(\mu \| \pi_{\text{QSD}})
$$

Therefore:

$$
\mathcal{I}(\mu \| \pi_{\text{QSD}}) \ge \frac{1}{C_{\text{LSI}}^{\text{Gibbs}}} D_{\text{KL}}(\mu \| \pi_{\text{QSD}})
$$

**Step 3 - Substitute into HWI**:

$$
\sqrt{D_{\text{KL}}(\mu \| \pi_{\text{QSD}})} \le W_2(\mu, \pi_{\text{QSD}}) \sqrt{\frac{1}{2C_{\text{LSI}}^{\text{Gibbs}}} D_{\text{KL}}(\mu \| \pi_{\text{QSD}})}
$$

**Step 4 - Simplify**:

$$
\sqrt{D_{\text{KL}}(\mu \| \pi_{\text{QSD}})} \le W_2(\mu, \pi_{\text{QSD}}) \cdot \frac{1}{\sqrt{2C_{\text{LSI}}^{\text{Gibbs}}}} \cdot \sqrt{D_{\text{KL}}(\mu \| \pi_{\text{QSD}})}
$$

Divide both sides by $\sqrt{D_{\text{KL}}}$ (assuming $D_{\text{KL}} > 0$):

$$
1 \le \frac{W_2(\mu, \pi_{\text{QSD}})}{\sqrt{2C_{\text{LSI}}^{\text{Gibbs}}}}
$$

Wait, this gives a bound in the wrong direction! Let me reconsider.

Actually, the HWI inequality is typically used in the form:

$$
D_{\text{KL}}(\mu \| \pi) \le W_2(\mu, \pi) \sqrt{2 \mathcal{I}(\mu \| \pi)}
$$

(squaring both sides of the original HWI).

**Step 5 - Correct application**:

For measures close to equilibrium, we use the **linearized HWI** (valid when $D_{\text{KL}} = O(\epsilon)$ small):

$$
D_{\text{KL}}(\mu \| \pi) \approx \frac{1}{2} \mathcal{I}(\mu \| \pi) \approx \frac{1}{2C_{\text{LSI}}^{\text{Gibbs}}} D_{\text{KL}}(\mu \| \pi)
$$

No wait, this is circular. Let me use the correct form.

**Correct Step 5 - Entropy change bound**:

For the revival operator $\Psi_{\text{revival}}$, we want to bound:

$$
D_{\text{KL}}(\Psi_{\text{revival}}^*\mu \| \pi_{\text{QSD}})
$$

By the data processing inequality for relative entropy:

$$
D_{\text{KL}}(\Psi_{\text{revival}}^*\mu \| \pi_{\text{QSD}}) \le D_{\text{KL}}(\mu \| \Psi_{\text{revival}}^{-1,*}\pi_{\text{QSD}})
$$

But $\Psi_{\text{revival}}$ is not invertible (it's a stochastic map with cloning).

**Alternative approach - Use Otto calculus**:

In optimal transport theory, the HWI inequality implies that for a Wasserstein contraction map $\Psi$ satisfying $W_2(\Psi_*\mu, \pi) \le (1 - \lambda) W_2(\mu, \pi)$:

$$
D_{\text{KL}}(\Psi_*\mu \| \pi) \le D_{\text{KL}}(\mu \| \pi) - \lambda C_{\text{HWI}}^{-1} W_2(\mu, \pi) + O(W_2(\mu, \pi)^2)
$$

Hmm, this also gives entropy DECREASE from Wasserstein contraction, which contradicts the idea that cloning EXPANDS entropy.

**Resolution - Cloning is not a gradient flow**:

The revival operator is NOT a Wasserstein gradient flow. It's a stochastic map that:
- Contracts Wasserstein (by Lemma 2.3)
- But may expand entropy (by introducing randomness in companion selection)

The HWI inequality bounds the MAGNITUDE of entropy change, not the sign.

**Correct formulation**:

For the cloning operator (killing + revival), the total entropy change is:

$$
D_{\text{KL}}(\Psi_{\text{clone}}^*\mu \| \pi_{\text{QSD}}) = D_{\text{KL}}(\Psi_{\text{revival}}^* \Psi_{\text{killing}}^* \mu \| \pi_{\text{QSD}})
$$

$$
\le D_{\text{KL}}(\Psi_{\text{killing}}^*\mu \| \pi_{\text{QSD}}) + \text{(revival entropy change)}
$$
...
The revival stage contracts Wasserstein (Lemma 2.3) but the HWI inequality tells us the entropy change from revival is bounded by:

$$
|\Delta D_{\text{KL}}^{\text{revival}}| \le C_{\text{HWI}} W_2(\mu, \pi_{\text{QSD}})
$$

Since revival involves stochastic companion selection, it can INCREASE entropy by up to this amount.
...
**Definition of $C_{\text{HWI}}$**:

The HWI constant is related to the log-Sobolev constant of $\pi_{\text{QSD}}$:

$$
C_{\text{HWI}} = \sqrt{2 C_{\text{LSI}}^{\text{Gibbs}}} = \sqrt{2/\kappa_{\text{conf}}} = O(1/\sqrt{\kappa_{\text{conf}}})
$$

This is N-uniform because $\kappa_{\text{conf}}$ is a property of the single-walker potential $U(x)$. ✓

---

### 2.5 Combined Cloning Entropy Bound

We now consolidate the killing and revival bounds into a single lemma for the full cloning operator.

:::{prf:lemma} Discrete Cloning Entropy Bound via HWI
:label: lem-discrete-cloning-entropy-bound
...
$$
D_{\text{KL}}(\Psi_{\text{clone}}^*\mu \| \pi_{\text{QSD}}) \le D_{\text{KL}}(\mu \| \pi_{\text{QSD}}) + C_{\text{kill}} \tau + C_{\text{HWI}} W_2(\mu, \pi_{\text{QSD}}) + C_{\text{clone}} \tau^2
$$
...
where:
- $C_{\text{kill}} = O(\beta V_{\text{fit,max}}^2)$ is the killing entropy expansion (N-uniform)
...
- $C_W = O(V_{\text{fit,max}})$ and $C_{\text{clone}} = O(\beta V_{\text{fit,max}}^2 + C_W)$ are second-order corrections (N-uniform)

**Proof**: Immediate combination of Lemmas 2.2, 2.3, and 2.4. ∎
:::

**Physical Interpretation**:
- **Killing** increases entropy by $O(\tau)$ (measure conditioning introduces randomness)
- **Revival** contracts Wasserstein by $O(\tau)$ (inelastic collisions)
- **HWI term** $C_{\text{HWI}} W_2$ couples the two effects: entropy expansion is controlled by Wasserstein contraction
- **Net effect**: Cloning expands entropy but with Wasserstein coupling (to be balanced by kinetic dissipation in Stage 3)

**N-Uniformity**: All constants are N-uniform by:
- Axiom EG-4 (bounded fitness)
- Theorem 0.2 (Keystone N-uniformity)
- Single-walker potential properties ($\kappa_{\text{conf}}$)
- Propagation of chaos (Theorem 0.3) ensures finite-N corrections are $O(1/N)$

✓

**Conclusion of Stage 2**: Cloning operator has controlled entropy expansion $O(\tau)$ plus Wasserstein coupling term $C_{\text{HWI}} W_2$, with Wasserstein contraction at rate $\kappa_x \tau$. ✓
