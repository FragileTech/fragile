...
class DeepDependencyAnalyzer:
    ...
    def _extract_proof_steps(
        ...
        for match in re.finditer(
            r"(?:By|Using|From|Since|Because|Via)\s+(?:the\s+)?([A-Z][a-z]+)\s+(\{prf:ref\}`[^`]+`)",
            proof_content,
            re.IGNORECASE,
        ):
        ...
        for pattern, assumption_type in assumption_patterns:
            for match in re.finditer(pattern, proof_content, re.IGNORECASE):
                context = proof_content[
                    max(0, match.start() - 100) : min(len(proof_content), match.end() + 100)
                ]
    ...
    def _infer_implicit_deps(self, directive: DirectiveInfo):
        ...
        if "d_{\\mathcal Y}^{\\mathrm{Sasaki}}" in content or "Sasaki metric" in content:
            ...
            has_explicit_ref = any(
                d.target_label == "def-sasaki-metric" for d in directive.explicit_deps
            )
    ...
    def build_dependency_graph(self):
        ...
        for label, directive in self.directives.items():
            ...
            for dep in all_deps:
                if (
                    dep.target_label in self.directives
                    or dep.target_label.startswith("def-")
                    or dep.target_label.startswith("axiom-")
                ):
        ...
        print(
            f"  Created {len(self.dependency_graph['nodes'])} nodes and {len(self.dependency_graph['edges'])} edges"
        )
    ...
    def _compute_analysis_stats(self) -> dict:
        ...
        most_depended = sorted(
            dependency_counts.items(), key=operator.itemgetter(1), reverse=True
        )[:10]
