...
**Diversity Channel Distribution**: The distribution $\mathcal{P}_{D(i)}$ of the diversity channel is an explicit sum over all companion maps produced by the sequential greedy pairing routine (including the self-pair when $k$ is odd or $k=1$). Each term factors into the probability of the pairing realization and a deterministic evaluation of the regularized distance statistic.

**Conditional Cloning Probability Field**: For every potential companion $j$, the cloning pipeline produces a random variable $\Pi(i|j)$ whose randomness comes from diversity pairing realizations; the cloning companion draw itself is independent. The family $\{\mathcal{P}_{\Pi(i|j)}\}_{j}$ is the discrete probability field controlling local exploration pressure ({prf:ref}`def-cloning-probability`).
...
*   $c_j$ is the companion of walker $j$ in the **diversity** companion map $M \in \mathcal{M}(\mathcal{A}(S_t))$; if $k$ is odd (or $k=1$), the leftover walker is mapped to itself, $c_j=j$.
*   $C_i^{\text{div}}$ is the random choice used by the diversity pairing routine.
*   $C_i$ is the **cloning** companion for walker $i$, drawn independently of $M$ from a separate kernel that excludes $i$ whenever possible (so $P(C_i=i)=0$ if $k>1$).
...
$$
\Theta_{\text{obs}} = \{ \lambda_{\text{alg}}, \epsilon_d, \epsilon_c, \epsilon_{\text{dist}}, \rho, \sigma_{\text{min}}, A, \eta, \alpha, \beta, p_{\max}, \epsilon_{\text{clone}}, \sigma_x, \alpha_{\text{restitution}}, c_{v\_\text{reg}}, R_{\text{pos}}(\cdot), U(\cdot), \gamma, \beta_{\text{kin}}, \Delta t \}.
...
The **sequential greedy pairing** algorithm produces a companion map $M$ of $\mathcal{A}(S_t)$. When $k$ is even, $M$ is a perfect matching. When $k$ is odd, the implementation returns an involution with one fixed point: the leftover walker is mapped to itself, $c_i=i$, which keeps the distance measurement well-defined because of the $\epsilon_{\text{dist}}$ regularizer. When walker $i$ is processed and chooses among the unpaired set $U \subseteq \mathcal{A}(S_t)$, the selection probability for $j \in U \setminus \{i\}$ is

$$
P(C_i^{\text{div}} = j \mid i, U; \lambda_{\text{alg}}, \epsilon_d) = \frac{\exp\left(-\frac{d_{\text{alg}}(i, j; \lambda_{\text{alg}})^2}{2\epsilon_d^2}\right)}{\sum_{\ell \in U \setminus \{i\}} \exp\left(-\frac{d_{\text{alg}}(i, \ell; \lambda_{\text{alg}})^2}{2\epsilon_d^2}\right)}.

$$
If $U \setminus \{i\} = \varnothing$ (e.g., $k=1$), the implementation sets $c_i=i$.

Let $\mathcal{I}(M)$ denote the ordered list of walkers processed as the "first" element of each pair. The probability of a pairing realization $M \in \mathcal{M}(\mathcal{A}(S_t))$ is the product of the sequential choices that realize those pairs, conditioned on the processing order used by the greedy algorithm (the fixed point, if present, contributes no factor):

$$
P(M \mid S_t; \lambda_{\text{alg}}, \epsilon_d) = \prod_{i \in \mathcal{I}(M)} P(C_i^{\text{div}} = c_i \mid i, U_i(M); \lambda_{\text{alg}}, \epsilon_d).
...
The deterministic reward of walker $j$ uses the positional reward potential $R_{\text{pos}}$ and the velocity regularization coefficient $c_{v\_\text{reg}}$:

$$
r_j(S_t; R_{\text{pos}}, c_{v\_\text{reg}}) = R_{\text{pos}}(x_j) - c_{v\_\text{reg}} \|v_j\|^2.

$$
The kinetic potential $U$ used by the Langevin operator may be different from $R_{\text{pos}}$; the reward channel does not assume a shared potential.

### 4.2. Reward Standardization

The global statistics follow the same form as Section 3.3, using the full alive set. For a localized regime, the same kernel $K_{\rho}$ is reused:
...
Given a diversity pairing realization $M$, the cloning score of walker $i$ relative to a cloning companion $j$ is ({prf:ref}`def-cloning-score`)
...
$$
determines the mean contribution of $j$ to the total cloning probability $\pi_{\text{clone}}(i \mid S_t)$. The cloning companion draw $C_i$ is **independent** of the diversity pairing $M$ and excludes $i$ whenever possible, so $P(C_i=i)=0$ if $k>1$. The total probability is

$$
\pi_{\text{clone}}(i \mid S_t) = \sum_{j \in \mathcal{A}(S_t)} P_{C_i}(j \mid S_t; \lambda_{\text{alg}}, \epsilon_c) \, \bar{p}(i \mid j),
...
$$
\mathcal{P}_{X'_i}(x' \mid S_t; \Theta_{\text{obs}}) = (1 - \pi_{\text{clone}}(i \mid S_t)) \cdot \delta(x' - x_i) + \sum_{j \in \mathcal{A}(S_t)} w_{ij}^{\text{joint}} \cdot \mathcal{N}(x'; x_j, \sigma_x^2 I_d),
...
When $k>1$, $P(C_i=i)=0$ so the self-centered Gaussian component drops out; when $k=1$, the only potential cloning companion is $i$ itself and its weight is determined by $\bar{p}(i \mid i)$.
...
$$
and obey $\sum_{j} w_{ij}^{\text{joint}} = \pi_{\text{clone}}(i \mid S_t)$. Each Gaussian component is therefore weighted by the probability of selecting $j$ as a companion and subsequently cloning against $j$.

### 6.3. Post-Cloning Velocity Distribution

Let $C_i=j$ and let $I_c$ denote the collision group associated with companion $c=j$ (so $i \in I_c$). The inelastic collision update ({prf:ref}`def-inelastic-collision-update`) gives

$$
V'_i = V_{\text{COM},c} + \alpha_{\text{restitution}}\,R_i^{\text{rot}}(u_i), \quad u_i = v_i - V_{\text{COM},c},
$$
...
$$
where the joint distribution $\mathcal{P}_{X'_i, V'_i}$ is induced by the cloning operator (positional marginal in Section 6.1, velocity update in Section 6.3).

### 7.2. Death Probability Field

Let $\mathcal{X}_{\mathrm{valid}} \subset \mathbb{R}^d$ be the viable domain ({prf:ref}`def-valid-state-space`). For each companion $j$ let $E_{ij}$ denote the event that $C_i=j$ and the cloning action indicator is 1. The conditional probability of exiting $\mathcal{X}_{\mathrm{valid}}$ after cloning to $j$ and applying the kinetic step is
...
| Reward potential | $R_{\text{pos}}(\cdot)$ | $[\text{dimensionless}]$ | Positional reward used in $r_j$ | Section 4.1 |
| Kinetic potential | $U(\cdot)$ | $[\text{dimensionless}]$ | Drives the Langevin dynamics | Section 7.1 |
...
| Spatial jitter | $\sigma_x$ | $[\text{distance}]$ | Standard deviation of positional perturbations during cloning | Section 6.1 |
| Restitution coefficient | $\alpha_{\text{restitution}}$ | $[\text{dimensionless}]$ | Inelastic collision contraction factor | Section 6.3 |
