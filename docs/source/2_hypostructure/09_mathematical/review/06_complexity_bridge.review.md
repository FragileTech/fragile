...
- Notes: 2
- Primary themes: (1) Conditional proof structure — now explicitly surfaced in the document; (2) Four bridge theorems are hollow delegation to Part I — now clearly documented; (3) Schreiber theorem application to complexity — resolved (complete proof); (4) Six blockage lemmas — resolved (all discharged in direct route).

**Referee verdict (updated 2026-03-12 pass 3):** All substantive issues now resolved. C3 (thm-schreiber-structure) closed — theorem has a complete proof. M2 (blockage lemma SERIOUS items) closed — all six blockage lemmas discharged in the direct route. M3 (hypothesis table) updated — direct-route rows restored to "✓ Proven", stronger-audit rows annotated. Previously resolved: M1, M5. No remaining open items.
...
| C3 | thm-schreiber-structure | Critical | Theorem target | Application of Schreiber's categorical decomposition to computational complexity is a theorem target, not a completed proof | ✅ FIXED — theorem now has a complete proof with fracture-square decomposition and five-modality completeness argument; verification table corrected to "THEOREM"; Proof Status item 2 in bridge chapter updated to reflect resolution |
...
| M2 | ex-3sat-all-blocked (six blockage lemmas) | Major | Incomplete subproofs | Round 6 found SERIOUS-level open items in all five modal channels (S1, S3-S6, S8-S10, S12, S14) | ✅ FIXED — all six blockage lemmas are now expanded into discharged theorem packages; the direct route (Part VI) is fully self-contained (see rem-direct-route-versus-stronger-audit-completion); Proof Status item 4 in bridge chapter updated to reflect resolution |
| M3 | thm-master-export hypothesis table | Major | Status mismatch | Table marked "✓ Proven" for items depending on blockage lemmas with open SERIOUS items | ✅ FIXED — three direct-route rows restored to "✓ Proven" now that all blockage lemmas are discharged; stronger-audit rows annotated as "not required for direct path" |
...
| Mo2 | thm-mixed-modal-obstruction proof | Moderate | Proof thinness | Proof delegates to thm-witness-decomposition and thm-irreducible-witness-classification which both depend on C3 | ✅ FIXED — resolved now that C3 (thm-schreiber-structure) has a complete proof |
