...
$\|D\psi(v)\|_{\mathrm{op}}\leq1$ for every $v\neq0$. Continuity at the origin
and the mean-value theorem yield the Lipschitz bound. The speed cap is obvious
from
\[
...
$L_{\mathrm{mf}}<\infty$, depending only on
$\nu_{\mathrm{visc}},k_*,k^*,\ell_{\mathrm{visc}}$, such that
\[
\|F[\mu](x,v)-F[\nu](x',v')\|
\leq
L_{\mathrm{mf}}
...
\]
Because $m_\mu(x)\geq k_*$ and $K$ is bounded and Lipschitz, the map
$x\mapsto A[\mu](x)$ is globally Lipschitz with constant bounded by a multiple of
$k_*^{-2}\|\nabla K\|_\infty\int(1+\|q\|)\mu(dy,dq)$. The same lower bound
controls differences of reciprocals:
\[
|m_\mu(x)^{-1}-m_\nu(x')^{-1}|
\leq
k_*^{-2}|m_\mu(x)-m_\nu(x')|.
\]
The Kantorovich--Rubinstein duality bounds both
$|m_\mu(x)-m_\nu(x')|$ and
$\bigl\|\int K(x,\cdot)q\,d\mu-\int K(x',\cdot)q\,d\nu\bigr\|$
by a constant times $\|x-x'\|+W_1(\mu,\nu)$. The estimate for
$F[\mu](x,v)-F[\nu](x',v')$ then follows from the decomposition
\[
F[\mu](x,v)-F[\nu](x',v')
=
...
\]
Because the kernel $K$ is $C^\infty$ and strictly positive, every weight
$\omega_{ij}(X)$ is smooth in $X$. Hence $b_N$ is locally Lipschitz on
$(\R^{2d})^N$. We therefore obtain a unique strong solution up to the explosion
time. To rule out explosion, define
\[
...
$(m_U,L_U,\gamma,\sigma,\nu_{\mathrm{visc}},k_*,k^*)$ and not on $N$. By
Gr\"onwall's lemma, $\sup_{t\leq T}\E\mathcal E_N(t)<\infty$ on every finite
interval, which excludes explosion. Hence the strong solution is global.

For the McKean--Vlasov equation, Lemma~\ref{lem:mf-lipschitz} yields Lipschitz
dependence of the drift on $(x,v,\mu)$ in the $W_1$ topology, while the force
has at most linear growth in $(x,v)$ under a finite second moment assumption on
$\mu$. A Picard iteration on the nonlinear SDE
\[
...
\begin{proof}
The finite-particle estimate is exactly the differential inequality derived in
the proof of Proposition~\ref{prop:wp}. For the mean-field equation, apply
It\^o's formula to $\|\bar X_t\|^2+\|\bar V_t\|^2$ along the nonlinear SDE.
Since
\[
\|F[f_t](x,v)\|
\leq
...
\]
Another application of Gr\"onwall's lemma finishes the proof.
\end{proof}
...
\begin{proof}
For fixed $X$, the SDE is an Ornstein--Uhlenbeck process with constant drift
matrix $A(X)$ and constant forcing $-b(X)$. Since
Lemma~\ref{lem:weighted-sym}(iv) yields
$\mathrm{Re}\,\mathrm{spec}(A(X))\subset[\gamma,\infty)$, the process is stable
and has a unique stationary Gaussian law with mean $m_X=-A(X)^{-1}b(X)$ and
covariance solving \eqref{eq:lyap}.

To bound the covariance, write $H(X):=D(X)^{1/2}\otimes I_d$ and
$A(X)=H(X)^{-1}\widetilde A(X)H(X)$ with $\widetilde A(X)$ symmetric positive
definite. The covariance admits the integral representation
\[
\Sigma_X
=
...
\frac{k^*}{k_*}.
\]
Taking the supremum over unit vectors $u$ proves the covariance bound.
\end{proof}
...
\begin{proof}
The Poincar\'e constant of a Gaussian law is the largest eigenvalue of its
covariance matrix. Apply Proposition~\ref{prop:gaussian}.
\end{proof}
...
\begin{proof}
The uncoupled semigroup factorizes:
$P_t^{N,0}=(P_t^{(1),0})^{\otimes N}$.
Relative entropy is additive on product measures, so the one-particle estimate
from Corollary~\ref{cor:backbone-gap} yields the same decay rate on
$\pi_1^{\otimes N}$.
\end{proof}
...
$k_*,k^*$, and $L_K$, uniformly in $N$. Consequently, when one differentiates
the interacting generator with respect to $x$, every extra term produced by the
viscous alignment field is bounded by
\[
C_{\mathrm{visc}}\sqrt{\mathcal B_N[h]\mathcal D_N[h]}
\]
for an $N$-independent constant $C_{\mathrm{visc}}$.
...
\]
Apply \eqref{eq:kl-finite} and take square roots.
\end{proof}
...
\begin{proof}
Total variation contracts under measurable pushforwards. The Wasserstein
contraction follows from Lemma~\ref{lem:squash}, since the observation map is
$1$-Lipschitz in velocity and the identity in position.
\end{proof}
...
\begin{proof}
Apply Corollary~\ref{cor:tv-finite} at time $m\tau$ and then push forward by
$S$ using Proposition~\ref{prop:squash-transfer}. The velocity bound is exactly
...
$1(1-1)/(2N)=0$, the total-variation distance is zero:
\[
...
\]
Taking expectations proves the identity.
\end{proof}
...
\]
The cone of probability densities in $H_w^1$ is a closed subset of the Banach
space $H_w^1$. Write the linear kinetic adjoint as
\[
\mathcal L_0^*\rho
=
-\nabla_x\cdot(v\rho)
...
Next we estimate the non-linear part. Lemma~\ref{lem:mf-lipschitz}, together
with the weighted Sobolev embedding
$H_w^1(\R^{2d})\hookrightarrow \mcP_1(\R^{2d})$ on bounded balls,
controls the mean-field force in terms of the underlying density:
\[
\|F[\rho_1]-F[\rho_2]\|_{L^\infty}
\leq
L_F\|\rho_1-\rho_2\|_{H_w^1}.
\]
Since $F[\rho](x,v)=\nu_{\mathrm{visc}}(A[\rho](x)-v)$ is affine in $v$ and
globally bounded in $x$ on bounded $H_w^1$-balls, one obtains
\[
\bigl\|\nabla_v\cdot(F[\rho_1]\rho_1-F[\rho_2]\rho_2)\bigr\|_{L_w^2}
\leq
L_{\mathrm{nl}}\|\rho_1-\rho_2\|_{H_w^1}
\]
for some finite constant $L_{\mathrm{nl}}$.
...
$1$. Then $\mathcal T$ is a strict contraction on the complete metric space of
probability densities in $H_w^1$. Banach's fixed-point theorem yields a unique
fixed point, and the fixed-point equation is exactly
...
\]
This identification mechanism is the stationary mean-field limit argument for
the kinetic core: the finite de Finetti reduction converts the discrete empirical
interaction into the mean-field weak formulation. The strong law of large
numbers under the product fibers $\mu^{\otimes N}$ and the continuity of the
interaction functional imply that $Q_\infty$ is concentrated on stationary weak
solutions of the McKean--Vlasov equation.
...
\begin{proof}
Fix $g\in C_c^\infty(\R^{2d})$ and lift it to $(\R^{2d})^N$ by
\[
G_N(z_1,\dots,z_N):=g(z_1).
\]
Applying the finite-particle LSI \eqref{eq:lsi-finite} gives
\[
\Ent_{\pi_N}(G_N^2)
\leq
C_{\mathrm{LSI}}
\int \Gamma_{\mathrm{hypo}}^N(G_N,G_N)\,d\pi_N.
\]
Because $G_N$ depends only on the first coordinate,
\[
...
\leq
C_{\mathrm{LSI}}
\int_{\R^{2d}} \Gamma_{\mathrm{hypo}}^\infty(g,g)\,d\pi_{N,1}.
\]
Theorem~\ref{thm:stationary-poc} gives $\pi_{N,1}\Rightarrow\pi_\infty$. Since
$g$ is smooth and compactly supported, both $g^2$ and $g^2\log g^2$ are bounded
and continuous, and so is $\Gamma_{\mathrm{hypo}}^\infty(g,g)$. Therefore
\[
\int g^2\,d\pi_{N,1}\to \int g^2\,d\pi_\infty,
\qquad
\int g^2\log g^2\,d\pi_{N,1}\to \int g^2\log g^2\,d\pi_\infty,
\]
and
\[
