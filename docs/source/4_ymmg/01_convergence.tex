...
Next we estimate the non-linear part. On the closed \(H_w^1\)-ball singled out
by the resolvent estimate, the first-moment bound is uniform, so the hypothesis
of Lemma~\ref{lem:mf-lipschitz} applies with one constant throughout the
argument. Because
\[
...
\]
This is the stationary weak mean-field identity for the limit mixture. Since the
test functions \(\phi\) are arbitrary, any measure \(\mu\) in the support of
\(Q_\infty\) must satisfy the stationary weak formulation of the McKean--Vlasov
equation; otherwise one could select \(\phi\) so that \(\Psi_\phi(\mu)\neq0\),
contradicting the averaged identity above.
