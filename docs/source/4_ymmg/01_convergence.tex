...
\]
Then \(\mathcal P_R\) is a closed subset of the Banach space \(H_w^1\), hence
complete. The weighted norm on \(\mathcal P_R\) gives a uniform first-moment
bound on that ball, so Lemma~\ref{lem:mf-lipschitz} applies there with one
constant throughout the argument.
...
\]
whose operator norm we denote by \(M_C\) and which satisfies \(M_C\to0\) as
$C\to\infty$. The positivity-preserving property of the kinetic semigroup
implies \(\mathcal T[\rho]\geq0\) whenever \(\rho\geq0\), and integrating the
defining equation over \(\R^{2d}\) shows
\[
\int_{\R^{2d}} \mathcal T[\rho]=1
\qquad\text{whenever}\qquad
\int_{\R^{2d}} \rho=1,
\]
because both the transport divergence and the velocity divergence have zero
integral. Thus \(\mathcal T\) maps the probability-density cone of \(H_w^1\)
into itself.
...
=
\nabla_v\cdot\bigl((F[\rho_1]-F[\rho_2])\rho_1\bigr)
+\nabla_v\cdot\bigl(F[\rho_2](\rho_1-\rho_2)\bigr).
\]
The uniform first-moment bound on \(\mathcal P_R\) and Lemma~\ref{lem:mf-lipschitz}
therefore give a finite constant \(L_{\mathrm{nl}}(R)\) such that
\[
\bigl\|\nabla_v\cdot(F[\rho_1]\rho_1-F[\rho_2]\rho_2)\bigr\|_{L_w^2}
\leq
L_{\mathrm{nl}}(R)\|\rho_1-\rho_2\|_{H_w^1}
\qquad
\forall \rho_1,\rho_2\in\mathcal P_R.
\]
Applying the resolvent bound therefore gives
\[
\|\mathcal T[\rho_1]-\mathcal T[\rho_2]\|_{H_w^1}
\leq
M_C\bigl(C+L_{\mathrm{nl}}(R)\bigr)
\|\rho_1-\rho_2\|_{H_w^1}.
\]
Because \(M_C\to0\) as $C\to\infty$, we may choose $C$ so large that
\[
\kappa_R:=M_C\bigl(C+L_{\mathrm{nl}}(R)\bigr)<1.
\]
The a priori resolvent estimate used to choose \(R\) also gives
\(\mathcal T[\mathcal P_R]\subseteq \mathcal P_R\), so \(\mathcal T\) is a
strict contraction on the complete metric space \(\mathcal P_R\). Banach's
fixed-point theorem yields a unique fixed point, and the fixed-point equation
is exactly \eqref{eq:mf-stationary-weak}. This fixed point is \(\pi_\infty\).
\end{proof}
\]
Applying the resolvent bound therefore gives
\[
