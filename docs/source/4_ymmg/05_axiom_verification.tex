...
(ii) the exact finite-dimensional gauge-fixed Euclidean path integral on that
same regulator, with rooted maximal-tree gauge fixing, identically unit
Faddeev--Popov determinant, and decoupled ghost sector;
...
not a distinct target theory.  It is the thermodynamic presentation of the same
gauge sector whose finite-regulator Wilson actions converge to the continuum
Yang--Mills functional on the stationary mean-field profile.
...
the thermodynamic presentation of the same deterministic-regulator gauge sector
and its continuum Yang--Mills limit.
...
Paper~3 quantizes the finite regulator exactly, fixes the gauge globally by rooted
maximal-tree gauge fixing, proves $\Delta_{\mathrm{FP}}=1$, and shows ghost
decoupling.  It also fixes the scalar witness sector on the same regulator and
transfers the mean-field logarithmic Sobolev constant to the product measures
supporting the thermodynamic observables \cite{quantization}.
...
\cite{axioms} remains in the background only as a separation-of-layers guide:
...
proved in the companion papers and in the direct arguments of the present
paper.
...
\item The flat-stationary realization satisfies the additional
realization-specific inputs that discharge the framework paper's conditional
OS2/OS3 clauses: exact hyperplane Markov factorization and a strict mean-field
spectral gap.
...
action is the stationary thermodynamic presentation of the same gauge sector
...
plaquette measure $\Gamma_\square^\infty$ constructed in the continuum gauge
paper.  Equivalently, by the global continuum-limit theorem of that same
paper, this thermodynamic presentation records the same gauge sector whose
finite-regulator Wilson actions converge to the continuum Yang--Mills
functional $S_{\mathrm{YM}}^{\mathrm{cont}}[F^\infty]$ on the stationary
mean-field curvature profile.
...
proves that the finite Wilson actions converge to the continuum Yang--Mills
functional $S_{\mathrm{YM}}^{\mathrm{cont}}[F^\infty]$ \cite{lattice}.
\item The quantization paper constructs the exact finite-dimensional Euclidean
path integral on the same regulator, proves finiteness of the partition integral,
...
paper.  Item \textnormal{(b)} is the main theorem and the
...
Yang--Mills identification.  The quantization paper provides the exact
gauge-fixed Euclidean integral, and the convergence paper provides the
stationary Markov and spectral-gap input.  The framework paper then serves only
...
The phrase ``bilocal mean-field Wilson action'' appears in the companion
continuum-gauge and quantization papers because the thermodynamic presentation
is written directly on the stationary law $\pi_\infty$.
...
$S_{\mathrm{YM}}^{\mathrm{cont}}[F^\infty]$ and the thermodynamic presentation
$S_{\mathrm W}^\infty$ records that same gauge sector on $\pi_\infty$;
\item the finite-regulator quantum theory is exact and unconditional on each
deterministic flat regulator, with exact gauge fixing and finite partition
integral;
...
the deterministic-regulator Wilson theory and its continuum Yang--Mills limit.
It is the stationary thermodynamic presentation of that same gauge sector.
\end{theorem}

\begin{proof}
The existence and explicit form of $S_{\mathrm W}^\infty$ are the thermodynamic
observable statements imported from the continuum gauge paper, while the
identification of the same finite-regulator Wilson sector with the continuum
functional $S_{\mathrm{YM}}^{\mathrm{cont}}[F^\infty]$ is exactly the content
of its global continuum-limit theorem.
...
stationary mean-field presentation of the same regulator theory whose Wilson
actions converge to the continuum Yang--Mills functional, with no appeal to the
obsolete trajectory-built lattice manuscript and with no extra local-continuum
...
bilocal action is the stationary presentation of the same deterministic-regulator
gauge sector identified in the companion paper with the continuum Yang--Mills
functional.
\end{enumerate}
...
variables whose support lies in $O$, first on the finite regulator and then in the
...
On the finite regulator, the local algebra is generated by finitely many edge and
...
a sum over plaquettes of the deterministic flat regulator, while the
quantization paper writes the fermionic and scalar sectors as sums over edges
and sites on that same regulator.
...
a Euclidean path integral.  In the $N\to\infty$ thermodynamic and continuum
limits, the continuum gauge paper proves both localized Wilson-to-Yang--Mills
convergence on compact windows and the global convergence
\[
...
of local plaquette contributions on the deterministic flat regulator, the
hyperplane decomposition persists in the limit.  Therefore the emergent
...
\begin{proof}
The continuum gauge and quantization papers define an explicit local $SU(3)$ gauge
...
The continuum gauge paper provides the Wilson plaquette action, the
thermodynamic presentation rewrites its exact thermodynamic observables on
$\pi_\infty$, and
...
The continuum gauge paper defines the deterministic flat regulator inside the
background $(\R^4,\delta)$ and proves that its Wilson sector converges to the
continuum Yang--Mills functional built only from the curvature tensor
$F^\infty_{\mu\nu}$ and the Euclidean metric.  The quantization paper keeps the
same flat regulator geometry and Haar product measure at fixed $N$.  Therefore
the limiting Schwinger functions are attached to a uniquely selected stationary
equilibrium on flat Euclidean space and depend only on the Euclidean metric and
the induced $SU(3)$ curvature profile.  This is the realization-specific
Euclidean covariance statement required for OS1.
\end{proof}
...
By Proposition~\ref{prop:imports}\textnormal{(c)}, the constructive dynamics
admit a stationary Markov path law.  Therefore the past and future are
...
framework paper, but here the needed factorization is supplied directly by the
stationary Markov path law of the realization itself.  OS2 is therefore proved
unconditionally in the present paper rather than left at framework level.
\end{remark}
...
\]
Here $T_t$ is the same stationary mean-field Markov semigroup whose generator
enters Proposition~\ref{prop:gap}; thus the Euclidean-time translation acting
on the boundary observables is represented directly by that semigroup in this
realization.  Applying the spectral-gap estimate from
...
The theory is nontrivial because the imported continuum-gauge and quantization papers
...
\section{What Papers 2 and 3 Did Not Prove and What the Present Paper Now Proves}
\label{sec:appendix-paper4}

\begin{proposition}[Disclaimers of the continuum-gauge and quantization papers are discharged here]
\label{prop:paper4-discharged}
The continuum gauge paper stopped at the deterministic regulator and
Wilson-to-Yang--Mills identification stage, and the quantization paper stopped
at exact finite-dimensional quantization.  Neither paper claimed either
Osterwalder--Schrader verification or a mass-gap theorem.  The present paper
discharges exactly those deferred steps.
\end{proposition}
...
Yang--Mills functional, but it does not verify OS0--OS4 or prove a Minkowski
mass gap.  The quantization paper constructs the exact gauge-fixed
finite-dimensional path integral, but again does not verify the
Osterwalder--Schrader axioms on the continuum theory and does not prove a mass
gap.  Those were the forward references left open by Papers~2 and~3.
...
Accordingly the earlier disclaimers remain correct for the continuum gauge and
quantization papers taken in isolation, but they no longer apply to the
completed series.
\end{proof}
...
G.~Duran Ballester,
\emph{From the Deterministic Regulator Data to a Gauge-Fixed Path Integral for
the Finite $SU(3)$ Yang--Mills Lattice Theory}, 2026.
