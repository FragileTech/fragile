...
\cite{continuumqft}.  The present paper completes the finite-dimensional data
package by adjoining the explicit fermionic and scalar actions written below,
and performs the
...
is well defined for the free massive scalar sector used in this paper.  We then solve the gauge-redundancy problem exactly by
...
support the thermodynamic observables.  The present paper supplies the exact
fixed-$N$ quantization, the local coordinate structure of its gauge measure,
and the explicit thermodynamic transfer of that equilibrium stiffness to the
quantized finite-lattice observables.
\end{abstract}
...
$S_{\mathrm W}$.  In the present paper we place on the same regulator graph the
...
summed over all admissible gauge fields and matter fields.  The present paper
...
graph by the displayed formulas in Definition~\ref{def:imported-package}.
\end{remark}
...
matter sector, one checks directly from the displayed formulas that
...
The same algebra is the one used throughout the finite-dimensional argument.
...
\cite{continuumqft}) is the finite-lattice thermodynamic continuum-limit
object on $\pi_\infty$.
...
data package $\mathcal Q^{(N)}$ to its quantum ensemble.
