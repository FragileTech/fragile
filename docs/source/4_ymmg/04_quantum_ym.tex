...
well defined for the free massive scalar witness sector fixed in the companion
lattice paper.  We then
...
determinant, and the scalar sector is shown to be exactly Gaussian with an
explicit lattice covariance.
...
\cite{convergence} proves this stationary mean-field limit, while the companion
lattice paper \cite{latticeqft} identifies the bilocal thermodynamic gauge
observables and the mean-field Wilson action $S_{\mathrm W}^\infty$ on
...
constant controls the finite product measures $\pi_\infty^{\otimes k}$ on
which the bilocal link and plaquette observables of \cite{latticeqft} are
defined.  The present paper supplies the exact fixed-$N$ quantization, the
local coordinate structure of its gauge measure, and the explicit thermodynamic
transfer of that equilibrium stiffness to the quantized mean-field observables.
\end{abstract}
...
\]
and the finite-dimensional actions imported from \cite{latticeqft}.  The
...
\pi_{N,1}\Rightarrow \pi_\infty,
\]
while \cite{latticeqft} formulates the corresponding bilocal gauge theory and
mean-field Wilson action $S_{\mathrm W}^\infty$ on $\pi_\infty$.
...
$\pi_\infty^{\otimes k}$ supporting the thermodynamic gauge observables.
\end{remark}
...
data, only loops in that explicit graph contribute.  The same-time part of that
graph is determined by the interaction data $\mathcal P_t$, which in the
trajectory construction of \cite{latticeqft} was built from the normalized
weights $\omega_{ij}^{\mathrm{link}}(t)$.
\end{remark}
...
measure model for the bilocal gauge observables that appear in the
$N\to\infty$ limit described in \cite{convergence,latticeqft}.
\end{remark}
...
\le
C_{\mathrm{LSI}}
\int_{\R^6}\Gamma_{\mathrm{hypo}}^\infty(g,g)\,\dd\pi_\infty
\]
for every smooth compactly supported $g:\R^6\to\R$, where
\[
...
$z_j=(x_j,v_j)\in\R^6$.  Then
\[
\Ent_{\Pi_k^\infty}(F^2)
\le
C_{\mathrm{LSI}}
\int_{(\R^6)^k}\Gamma_{\mathrm{hypo}}^{(k)}(F,F)\,\dd\Pi_k^\infty
\]
for every smooth compactly supported $F:(\R^6)^k\to\R$.
In particular, the bilocal link observables and plaquette observables of
\cite{latticeqft}, which are evaluated on $\pi_\infty^{\otimes2}$ and
...
\[
z=(z',z_k)\in (\R^6)^{k-1}\times\R^6.
\]
For a smooth compactly supported $F:(\R^6)^k\to\R$, set
\[
F_{z_k}(z'):=F(z',z_k),
\qquad
m(z_k):=\int_{(\R^6)^{k-1}} F(z',z_k)^2\,\dd\Pi_{k-1}^\infty(z'),
\qquad
G(z_k):=m(z_k)^{1/2}.
\]
...
\Ent_{\Pi_k^\infty}(F^2)
=
\int_{\R^6}\Ent_{\Pi_{k-1}^\infty}(F_{z_k}^2)\,\dd\pi_\infty(z_k)
+\Ent_{\pi_\infty}(G^2).
\]
...
C_{\mathrm{LSI}}
\sum_{j=1}^{k-1}
\int_{(\R^6)^k}\Gamma_{\mathrm{hypo},j}^\infty(F,F)\,\dd\Pi_k^\infty.
\]
...
bounded, hence $G_\varepsilon$ is smooth and bounded on $\R^6$.  Let
\[
M:=
\begin{pmatrix}
cI_3 & bI_3\\
...
$\chi_R\in C_c^\infty(\R^6)$ such that
\[
...
Set
\[
H_{\varepsilon,R}:=\chi_R G_\varepsilon\in C_c^\infty(\R^6).
\]
...
\le
C_{\mathrm{LSI}}
\int_{\R^6}\Gamma_{\mathrm{hypo}}^\infty(H_{\varepsilon,R},H_{\varepsilon,R})\,
\dd\pi_\infty.
\]
...
\le
C_{\mathrm{LSI}}
\int_{\R^6}\Gamma_{\mathrm{hypo}}^\infty(G_\varepsilon,G_\varepsilon)\,\dd\pi_\infty.
\]
...
\le
C_{\mathrm{LSI}}
\int_{(\R^6)^k}\Gamma_{\mathrm{hypo},k}^\infty(F,F)\,\dd\Pi_k^\infty.
\]
...
The thermodynamic link and plaquette observables of \cite{latticeqft} are
integrated against bounded indicators $\chi_t$ and $\chi_\square$ on the
product spaces $(\R^6)^2$ and $(\R^6)^4$.  Since
\[
...
Proposition~\ref{prop:thermo-lsi-transfer}; no additional thermodynamic scalar
measure hypothesis is needed at that step.
\end{remark}
...
product measure were defined, and the partition function was proved finite for
the free massive scalar witness sector imported from \cite{latticeqft}.  The
...
scalar sector is an exact Gaussian field with stiffness operator
$L_\eta+m_\phi^2I$, and the bilocal mean-field action $S_{\mathrm W}^\infty$
defined on the stationary measure $\pi_\infty$ provides the correct
thermodynamic-limit continuum theory.
...
\pi_{N,1}\Rightarrow \pi_\infty,
\]
while \cite{latticeqft} identifies the bilocal action
$S_{\mathrm W}^\infty$ and the associated thermodynamic gauge observables on
$\pi_\infty$.  The present paper supplies the exact finite-dimensional
quantization step on the trajectory-induced lattice, together with the local
Haar-coordinate analysis and the explicit gauge fixing needed to pass from the
data package $\mathcal Q$ to its quantum ensemble.
