...
gauge-invariant.
In the $N\to\infty$ thermodynamic limit, scaled link and plaquette sums converge
in probability to integrals against the deterministic stationary mean-field law
$\pi_\infty$.
The limit theory is a bilocal gauge theory: the connection
\[
A(z,z'):=\frac12\sum_{a=1}^8
\operatorname{Re}\!\bigl(c(z)^\dagger\lambda_a\bigl(c(z')-c(z)\bigr)\bigr)\lambda_a,
\qquad z,z'\sim\pi_\infty,
\]
the parallel transport $U(z,z'):=\exp(ig_3 A(z,z'))$, and the mean-field Wilson
action
\[
S_{\mathrm W}^\infty
=\int\!\!\int\!\!\int\!\!\int
\Bigl(1-\tfrac13\operatorname{Re}\operatorname{tr} U(z_1,z_2)U(z_2,z_3)U(z_3,z_4)U(z_4,z_1)\Bigr)
\,d\Gamma_\square^\infty(z_1,z_2,z_3,z_4)
\]
are all defined directly on the continuous mean-field measure, with no lattice
re-discretization required.
Path-integral quantization and axiom verification are handled in later papers.
\end{abstract}
...
This section supplies all the formal definitions and proofs required for a
...
The thermodynamic graph and plaquette limits are unconditional consequences of
the companion convergence paper together with the present graph construction.
The mean-field result is stated in Section~\ref{sec:discussion}: under the
$N\to\infty$ thermodynamic limit, the Wilson action converges to a bilocal
integral functional defined directly on the stationary mean-field law.
...
interpretation, the thermodynamic graph and plaquette limits, the bilocal
mean-field Yang--Mills action arising in the $N\to\infty$ regime, and quantization.
...
We write
\[
\mathfrakh:=\{H\in\C^{3\times3}:H^\dagger=H,\ \tr H=0\}
\]
for the corresponding Hermitian generator space, so that $i\mathfrakh=\mathfrak{su}(3)$.
\end{definition}
...
\end{proposition}
\begin{proof}
Each coefficient $\Re(\mathbf c_x^\dagger\lambda_a(\mathbf c_y-\mathbf c_x))$ is real.
...
\begin{remark}
There are no oriented self-loops in $\mcE$.
Interaction links exclude $(i,i)$ by definition, and temporal links connect
distinct times $t$ and $t+1$.
...
\[
F_P:=\frac{1}{i g_3 A_P}\log U_3(P)\in\mathfrakh.
\]
\end{definition}
...
Fix $m_F>0$, and let $m_\phi>0$ be the scalar witness mass from
Definition~\ref{def:scalar-mass}.
...
The scalar sector is the free massive witness field canonically tied below to
the stationary mean-field stiffness scale of the companion convergence paper.
\end{remark}
...
\mathfrak M_{[0,S]}^N
:=
\frac1N\sum_{i=1}^N \delta_{Z_i^N(\cdot)}
\in \mathcal P\bigl(C([0,S];\R^6)\bigr).
\]
Then
\[
\mathfrak M_{[0,S]}^N
...
\Pi_{[0,S]}^\infty
\qquad
\text{in }\mathcal P\bigl(C([0,S];\R^6)\bigr).
\]
Equivalently, for every bounded continuous path functional
$\Psi:C([0,S];\R^6)\to\R$,
\[
\frac1N\sum_{i=1}^N \Psi\!\bigl(Z_i^N(\cdot)\bigr)
\xrightarrow[N\to\infty]{\mathbb P}
\int_{C([0,S];\R^6)} \Psi(\gamma)\,\Pi_{[0,S]}^\infty(d\gamma).
\]
In particular, for every integer $0\le t<S$,
\[
M_t^N
...
Applying the same finite de~Finetti reduction used in the companion
convergence paper, now on the Polish space $E$, yields a mixing measure
$\widehat Q_N\in\mathcal P(\mathcal P(E))$ with canonical choice
\[
\widehat Q_N=\mathrm{Law}(\mathfrak M_{[0,S]}^N),
\]
...
Stationarity gives the same uniform second-moment bounds at every time as in
the companion convergence paper, and the underdamped Langevin coefficients have
at most linear growth with Brownian noise only in velocity.
Standard Kolmogorov estimates therefore give tightness of
$\mathrm{Law}(Z_1^N(\cdot))$ on $E$, hence tightness of $\{\widehat Q_N\}_{N\ge2}$ on
...
For a measure $\mu\in\mathcal P(\R^6)$, write
\[
...
\[
\phi(Z_i^N(v))-\phi(Z_i^N(u))
-\int_u^v
\Bigl[
\mathcal L_{\mathfrak M_{[0,S],s}^N}\phi(Z_i^N(s))
+\bigl(F_i^N(s)-F[\mathfrak M_{[0,S],s}^N](Z_i^N(s))\bigr)
\cdot\nabla_v\phi(Z_i^N(s))
\Bigr]\,ds
\]
is a martingale.
Averaging over $i$ and multiplying by $G(Z_i^N|_{[0,u]})$ gives zero
expectation.
Here
\[
\mathfrak M_{[0,S],s}^N:=(\operatorname{ev}_s)_\#\mathfrak M_{[0,S]}^N
=
\frac1N\sum_{j=1}^N \delta_{Z_j^N(s)}.
\]
The force-defect estimate from the companion convergence paper, integrated over
$s\in[u,v]$, shows that the second term inside the time integral contributes
only $O(N^{-1/2})$.
Passing to the limit and using continuity of $\mu\mapsto F[\mu]$ on
moment-bounded classes, every path law $\eta$ in the support of
...
Its time-$0$ marginal is the limit of the first marginals $\pi_{N,1}$ and
therefore equals $\pi_\infty$ by the stationary propagation-of-chaos theorem of
the companion paper.
The McKean--Vlasov SDE is well posed for every initial law with finite second
moment, so there is a unique path law with initial law $\pi_\infty$.
Since $\pi_\infty$ is stationary, that unique law is exactly
$\Pi_{[0,S]}^\infty$.
Hence $\widehat Q_\infty=\delta_{\Pi_{[0,S]}^\infty}$.
Every subsequence has the same limit, so
\[
\mathrm{Law}(\mathfrak M_{[0,S]}^N)\Rightarrow \delta_{\Pi_{[0,S]}^\infty},
\]
which is equivalent to the stated convergence in probability.
The final claim follows by the continuous mapping theorem for the evaluation map
$\operatorname{ev}_{t,t+1}$.
\end{proof}
...
Because $M_t^N$ converges in probability to the deterministic measure
$\Lambda_t^\infty$ by Theorem~\ref{thm:stationary-path-poc},
...
The companion convergence paper proves the one-time stationary propagation of
chaos used in Theorem~\ref{thm:edge-measure-limit}; the present
Theorem~\ref{thm:stationary-path-poc} upgrades that control to stationary path
space, and Theorem~\ref{thm:plaquette-measure-limit} is then the corresponding
...
In the $N\to\infty$ thermodynamic regime the discrete interaction graph
disappears and the relevant geometric objects are the stationary mean-field law
$\pi_\infty$ and its natural product measures.
The bilocal gauge fields and the Wilson action are defined directly on the
continuous probability space
$(\operatorname{supp}\pi_\infty, \pi_\infty)$.

\begin{definition}[Bilocal color connection]
\label{def:bilocal-connection}
Let $\pi_\infty$ be the stationary mean-field law from the companion convergence
paper~\cite{convergence}, and let $c:\operatorname{supp}\pi_\infty\to\C^3$ be
the color field inherited from Definition~\ref{def:colors}.
For $\pi_\infty$-almost every pair $(z,z')$, define the
\emph{bilocal $\mathfrak{su}(3)$-valued connection}
\[
A(z,z')
:=
\frac12\sum_{a=1}^8
...
\[
U(z,z'):=\exp\!\bigl(ig_3 A(z,z')\bigr)\in SU(3).
\]
For $\pi_\infty^{\otimes 4}$-almost every quadruple $(z_1,z_2,z_3,z_4)$, the
\emph{mean-field Wilson plaquette} is
\[
W(z_1,z_2,z_3,z_4)
:=U(z_1,z_2)\,U(z_2,z_3)\,U(z_3,z_4)\,U(z_4,z_1)\in SU(3).
\]
\end{definition}

\begin{definition}[Mean-field plaquette measure]
\label{def:mf-plaquette-measure}
Let $\chi_\square:\operatorname{supp}\pi_\infty^{\otimes 4}\to\{0,1\}$ be the
label-invariant quadruple indicator inherited from Definition~\ref{def:edge-measure}
by the thermodynamic limit of Theorem~\ref{thm:plaquette-measure-limit}.
The \emph{mean-field plaquette measure} is the positive measure on
$\operatorname{supp}\pi_\infty^{\otimes 4}$ defined by
\[
d\Gamma_\square^\infty(z_1,z_2,z_3,z_4)
:=
\chi_\square(z_1,z_2,z_3,z_4)\,
\pi_\infty(dz_1)\,\pi_\infty(dz_2)\,\pi_\infty(dz_3)\,\pi_\infty(dz_4).
\]
\end{definition}

\begin{theorem}[Mean-field Wilson action]
\label{thm:mf-wilson}
The \emph{mean-field Wilson action} is
\[
S_{\mathrm W}^\infty
:=
\int\!\!\int\!\!\int\!\!\int
\Bigl(1-\tfrac13\operatorname{Re}\operatorname{tr}
W(z_1,z_2,z_3,z_4)\Bigr)
\,d\Gamma_\square^\infty(z_1,z_2,z_3,z_4).
\]
By Theorem~\ref{thm:plaquette-measure-limit}, in the $N\to\infty$ thermodynamic
limit,
\[
S_{\mathrm W}^{(N)}
\xrightarrow[N\to\infty]{\mathbb P}
S_{\mathrm W}^\infty.
\]
The limit action $S_{\mathrm W}^\infty$ is gauge-invariant under simultaneous
$SU(3)$ rotations $c(z)\mapsto\Omega\,c(z)$ for any fixed $\Omega\in SU(3)$,
by the same argument as Proposition~\ref{prop:wilson-gauge}.
\end{theorem}
\begin{proof}
Apply Theorem~\ref{thm:plaquette-measure-limit} with $\Phi(z_1,z_2,z_3,z_4)
:=1-\tfrac13\operatorname{Re}\operatorname{tr}W(z_1,z_2,z_3,z_4)$, which is
bounded and continuous on $\operatorname{supp}\pi_\infty^{\otimes 4}$.
The convergence in probability follows immediately.
Gauge invariance is inherited from the finite-$N$ Wilson action
(Proposition~\ref{prop:wilson-gauge}) by passage to the limit.
\end{proof}

\begin{remark}[Thermodynamic regularization]
\label{rem:no-lattice-spacing}
The mean-field action $S_{\mathrm W}^\infty$ is defined entirely within the
probability space $(\operatorname{supp}\pi_\infty, \pi_\infty)$.
Its regularization is provided by the viscous kernel length scale
$\ell_{\mathrm{visc}}$ and the smooth Gaussian kernel $K_\rho$.
The asymptotic regime is the thermodynamic limit $N\to\infty$:
the continuous stationary measure $\pi_\infty$ \emph{is} the spacetime geometry
of the mean-field theory.
\end{remark}

\subsection{LSI-induced scalar witness sector}

The gauge sector has a genuine thermodynamic limit on $\pi_\infty$.
For the scalar sector used in the quantization paper, we now make the same
thermodynamic input explicit and fix a canonical free massive witness field.

\begin{definition}[Mean-field scalar witness observable]
\label{def:scalar-witness}
Let $\pi_\infty$ be the stationary mean-field law from the companion
convergence paper~\cite{convergence}.  Define the scalar witness observable by
\[
\sigma_\phi(z):=\|F[\pi_\infty](z)\|,
\qquad
z=(x,v)\in\R^6,
\]
where $F[\pi_\infty](x,v)=\nu_{\mathrm{visc}}(A[\pi_\infty](x)-v)$ is the
mean-field viscous drift from the convergence paper.
\end{definition}

\begin{proposition}[Thermodynamic control of the scalar witness]
\label{prop:scalar-witness}
The observable $\sigma_\phi$ is globally Lipschitz on $\R^6$ and belongs to
$L^2(\pi_\infty)$.  More precisely, if $L_{\mathrm{mf}}$ is the finite constant
supplied by the mean-field alignment-field Lipschitz estimate of
\cite{convergence}, evaluated on the
single-law class $\{\pi_\infty\}$, then
\[
|\sigma_\phi(z)-\sigma_\phi(z')|
\le
L_{\mathrm{mf}}\|z-z'\|
\qquad
\forall z,z'\in\R^6.
...
|\sigma_\phi(z)-\sigma_\phi(z')|
\le
\|F[\pi_\infty](z)-F[\pi_\infty](z')\|.
\]
Applying the mean-field alignment-field Lipschitz estimate of \cite{convergence} with
$\mu=\nu=\pi_\infty$ gives the stated Lipschitz bound.

To prove square integrability, fix $z_0=(0,0)\in\R^6$.  The Lipschitz estimate
implies
\[
\sigma_\phi(z)\le \sigma_\phi(z_0)+L_{\mathrm{mf}}\|z-z_0\|
\le
\sigma_\phi(z_0)+L_{\mathrm{mf}}\|z\|.
\]
The stationary propagation-of-chaos theorem of~\cite{convergence}, together
with the uniform second-moment estimate proved there for the stationary
marginals, implies that $\pi_\infty$ has finite second moment on $\R^6$.
Therefore $\sigma_\phi\in L^2(\pi_\infty)$.
\end{proof}

\begin{definition}[Scalar witness mass]
\label{def:scalar-mass}
Let $C_{\mathrm{LSI}}<\infty$ be the mean-field logarithmic Sobolev constant of
the companion convergence paper~\cite{convergence},
and let $L_{\mathrm{mf}}$ be as in Proposition~\ref{prop:scalar-witness}.
Define the scalar witness mass by
\[
m_\phi:=\bigl(1+C_{\mathrm{LSI}}L_{\mathrm{mf}}^2\bigr)^{-1/2}>0.
\]
\end{definition}

\begin{remark}[Interpretation of the scalar witness sector]
\label{rem:scalar-witness}
The scalar matter field in Definition~\ref{def:matter} is therefore not an
arbitrary local-potential sector.  It is the canonical free massive scalar
witness attached to the thermodynamic stiffness scale already certified by the
mean-field LSI on $\pi_\infty$.  No additional scalar local potential is
introduced anywhere in the series.
\end{remark}
...
Because the scalar action is free massive with $m_\phi>0$, the scalar sector is
already Gaussian and therefore finite before any further gauge promotion.
...
\item Define the $SU(3)$ link transport $U_3(e)=\exp(i\Theta_e)$ by projecting
      ...
      generator space $\mathfrakh$ via the Gell-Mann basis.
...
\item Prove the $N\to\infty$ thermodynamic mean-field limit for both link and
      plaquette observables, and define the bilocal mean-field Yang--Mills action
      $S_{\mathrm W}^\infty$ directly on the stationary measure $\pi_\infty$.
\end{enumerate}
...
deterministic mean-field limit for link observables in the sense of
Theorem~\ref{thm:edge-measure-limit}, and the canonical plaquettes have the
analogous deterministic mean-field limit by
Theorems~\ref{thm:stationary-path-poc} and~\ref{thm:plaquette-measure-limit}.
In the thermodynamic regime, Theorem~\ref{thm:mf-wilson} identifies the limit of
the discrete Wilson action as the bilocal mean-field action $S_{\mathrm W}^\infty$
on the stationary measure $\pi_\infty$, with regularization provided by the
viscous kernel $K_\rho$.

Two major extensions are recorded separately:
(i) path-integral quantization of the mean-field theory with the bilocal action
$S_{\mathrm W}^\infty$ and the Markov-chain measure on the interaction graph;
(ii) axiom verification --- checking that $\mathcal Q$ satisfies the
Osterwalder--Schrader axioms evaluated on the integral operator of the continuous
mean-field law.
