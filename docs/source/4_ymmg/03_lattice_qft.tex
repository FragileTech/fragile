...
No lattice spacing $a_N\to0$ limit is needed: the bilocal gauge fields and the
Wilson action are defined directly on the continuous probability space
...
\begin{remark}[No lattice spacing required]
...
Unlike the standard Wilson action on a periodic lattice, it does not involve a
lattice spacing $a$ or a limit $a\to0$; the regularization is provided by the
viscous kernel length scale $\ell_{\mathrm{visc}}$ and the smooth Gaussian
kernel $K_\rho$.
The thermodynamic limit $N\to\infty$ replaces the continuum limit $a\to0$: the
continuous stationary measure $\pi_\infty$ \emph{is} the spacetime geometry of
...
viscous kernel $K_\rho$ rather than a lattice spacing.
