...
Under an admissible mean-field refinement regime with a smooth color-field limit,
shape-regular plaquettes, explicit plaquette-count control, and plaquette-density
convergence, the discrete Wilson
...
geometric refinement hypotheses on the companion graph, the same data recover
...
There exists a compact smooth embedded four-dimensional manifold $M\subset\R^4$ with
...
\]
\item \textbf{Uniform plaquette count.}
There exists $C_\square<\infty$ such that
\[
|\mathcal C_\square^{(N)}|\le C_\square\,a_N^{-4}
\qquad\text{for all }N.
\]
\item \textbf{Bivector density convergence.}
For every continuous scalar test function $f$ on $M$,
\[
\sum_{P\in\mathcal C_\square^{(N)}}
...
Define the Hermitian traceless gauge potential on $M$ by
\[
...
U_3^{(N)}(e_k)
=
\exp\!\Bigl(i g_3\,\xi_{e_k}^{(N),\mu}A_\mu(\Xi^{(N)}(s(e_k)))+O(a_N^2)\Bigr)
\]
...
\]
Taylor expansion of $A_\mu(\Xi^{(N)}(s(e_k)))$ around the base point
$z_P^{(N)}$ gives
\[
A_\mu(\Xi^{(N)}(s(e_k)))
=
A_\mu(z_P^{(N)})
+\delta_k^{(N),\alpha}\partial_\alpha A_\mu(z_P^{(N)})
+O(a_N^2),
\]
where $\delta_k^{(N)}:=\Xi^{(N)}(s(e_k))-z_P^{(N)}=O(a_N)$ uniformly in $k$.
Applying the Baker--Campbell--Hausdorff formula twice, using the closed-loop
identity above, and collecting the first-order Taylor terms together with the
quadratic commutator terms yields
\[
\log U_3^{(N)}(P)
=
i g_3\,\Sigma_P^{(N),\mu\nu}
...
\[
U_3^{(N)}(P)=\exp\!\bigl(i g_3 B_P^{(N)}+E_P^{(N)}\bigr),
\qquad
B_P^{(N)}:=\Sigma_P^{(N),\mu\nu}F_{\mu\nu}(z_P^{(N)}).
\]
Here $E_P^{(N)}\in i\mathfrakh$ and $\|E_P^{(N)}\|=O(a_N^3)$ uniformly.
...
order-$a_N^3$ correction in the exponent is skew-Hermitian and traceless, so its
contribution to the real part of the trace starts only in mixed terms of order
$a_N^5$.
...
$|\mathcal C_\square^{(N)}|\le C_\square a_N^{-4}$, and hence
\[
...
\frac14\int_M \tr(F_{\mu\nu}F^{\mu\nu})\,d\mathrm{vol}_g.
\]
Proposition~\ref{prop:refinement-additional} shows that the geometric
refinement regime needed for this last step is genuinely extra structure on the
companion graph and is not implied by propagation of chaos alone.
