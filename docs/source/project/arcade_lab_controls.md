...
algorithm, a crowd of games that are only being considered. A **walker** is a
...
It is not just a dot wandering over a map.
...
planner may spend a long time examining futures without moving the committed
game by one frame. Then it chooses an action or path, plays it, and searches
...
|---|---|
| Which future is being copied? | **Virtual reward**, not **Cumulative reward**. |
...
can be extended. Only leaves selected for cloning step in a particular graph
update; the tree can therefore grow while the number of active leaves remains
...
committed game, but do not commit those candidate states. FMC chooses the most
represented first action among surviving search branches, then advances the
committed game by that one action and replans. Jump Wave can execute a common
initial path shared by the surviving branches, or the entire winning branch
when its consensus option is disabled. In both cases, the bright game view is
the committed emulator, not the best search snapshot.
:::

:::{div} feynman-added
| Algorithm | Population meaning | What is committed to the played game? | Special readout |
|---|---|---|---|
| **Wave** | Fixed `N` walkers; all act each iteration. | Nothing is committed; the leading walker is displayed. | Best walker screen and swarm plots. |
| **Graph** | `N` initial walkers and minimum leaves; nodes grow up to **Max walkers**. | Nothing is committed; the leading walker is displayed. | `Nodes / leaves / stepped`, plus parent lines on the map. |
...
sends only the selected action or path back to the game being played.
:::
...
None of those copies has changed the emulator on the table. Only after the
planner selects an executable action sequence does the committed game advance.

This is why **Max reward** can rise while the displayed Sonic or Mario screen
does not move during several search iterations. The search is making progress
in possible futures. The separate **Played frames** counter moves only when the
committed game executes an action. A search result can also be good without
being selected: the planner's rule is based on the algorithm, not simply on
showing the single highest-reward snapshot.
:::
...
horizon is a depth in action edges, not a number of seconds. The actual time
covered by an edge depends on its sampled `dt`.
...
the first edge, there is no prefix to play, so Jump Wave keeps searching. A
single surviving branch shares its path with itself; agreement does not require
different walkers to make the same action by coincidence.
...
must be at least the normal horizon. If no shared prefix appears by that limit,
the implementation falls back to one positive-duration action from the best
available branch. If all search walkers are dead, it likewise uses a one-action
best-path fallback when a path exists. With the checkbox off, Jump Wave executes
the selected best branch's full recorded path. Each selected action still needs
an execution step; a long path is not silently played in one browser callback.
:::
...
| **Stop at first bifurcation** | Checked | Jump Wave only | Live; checked uses the shared prefix, unchecked uses the full selected best path. |
...
Changing a planner setting is not a full game reset. It invalidates the queued
...
far apart. It is not necessarily what the human sees, and it is not the reward.
...
Coords is not a magical form of vision. It is a choice to compare the pieces of
state that matter for reachability: position, momentum, room, level, or other
game variables. Mario, Sonic, and dedicated Montezuma default to it; generic
Atari defaults to RAM because there is no single coordinate schema valid for
every Atari ROM. In generic Atari, selecting Coords deliberately aliases RAM.
...
| **Generic Atari / ALE** | RAM | `128` values | Dimensions reported by ALE; the common frame is `160×210` | Same `128`-value RAM vector; no generic coordinate tuple. |
...
machine has many pixels, not because it has understood the picture. A Coords
observation is small because the adapter has selected a few RAM-derived
variables. The reward adapter still runs separately. Switching from RAM to
Coords does not turn score into distance reward, and switching to RGB does not
make the algorithm perform learned visual recognition.
...
cap on graph nodes, not just on currently stepping leaves. The UI chooses a
...
cap may be lower when the browser's WebAssembly memory cannot hold another full
emulator state. The ready message reports that effective cap. Sonic's smaller
default is intentional: Genesis state blobs are comparatively expensive.
...
to zero to disable that buffer. Graph has no elite slider; its best state stays
in the retained tree rather than being placed in a separate elite slot.
:::
...
and dedicated Montezuma. The cloning operation does not use that number alone.
It builds a **virtual reward**, the selection signal that says which timeline
should be copied.
...
forming the product, so these are not exponents on raw pixel distances or raw
score units. A zero coefficient removes that factor's variation. Higher
...
The rescaling is asymmetric: a signal with no spread contributes the neutral
...
spread changes even if the underlying game score does not. A low-fitness walker
...
:::{div} feynman-prose
Visit controls appear only for **Coords** on Mario, Sonic, and dedicated
Montezuma. The implementation maintains visit counts even when their reward
term is switched off. That makes a useful ablation possible: turn the term off,
keep collecting the history, then turn it back on without pretending the past
was unvisited.
...
larger visit factor in virtual reward. Off makes the visit factor neutral; it
does not erase the counters. **Visit pooling (px)** defaults to `5`, with range
...
count in fitness or the heatmap. A larger window makes novelty coarser; changing
it does not throw away the per-pixel history.

**Erase coef** defaults to `0.05`, with range `0`–`1` in steps of `0.01`. At
each iteration stored counts decay by this coefficient. Zero means no
forgetting; a larger coefficient lets old exploration become novel again more
quickly. The visit grid is part of the current run, so **Reset** clears it.
...
two are related views, not the same data structure.
:::

:::{div} feynman-prose
There is a subtle but important separation here. A Graph node is a full emulator
state with an ancestry link. A visit cell is only a count attached to a
coordinate key. The graph can remember a branch that later becomes dead, while
the visit grid can say that a region has been explored many times. Neither view
is a complete policy, and neither replaces the committed game in a planner.
:::
...
changes rewards earned from the next transitions onward. It does not rewrite
the cumulative reward already carried by a walker, so changing a weight does
not retroactively change an old plot point or make an old elite's history mean
something new. For a planner, changing reward weights also discards the pending
search/trajectory and replans from the current committed game; it does not
rewind that game.
...
| Mario | **Flag bonus** (`flag`) | `500`; `0`–`2000`, step `10` | One-off reward when the flagpole is grabbed. It does not terminate this wrapper. |
| Mario | **Area bonus** (`area`) | `100`; `0`–`500`, step `5` | One-off reward for entering a previously unseen pipe, warp, bonus, or intro area in the stage. |
| Sonic | **X progress** (`dx`) | `1`; `0`–`5`, step `0.1` | Signed horizontal progress. It is disabled while a boss is loaded; implausible jumps beyond the guard are ignored. |
...
| Sonic | **Extra-life bonus** (`life`) | `1000`; `0`–`5000`, step `50` | Reward for gaining a life, including a 1-up or a 100-ring life. Life loss is terminal for that walker, not a positive term. |
...
| Montezuma | **New-room bonus** (`room`) | `500`; `0`–`2000`, step `10` | One-off reward for entering a new one of the 24 rooms in a lineage on the current temple level. No bonus during dying or for terminal room 8. |
| Generic Atari | — | No reward-term sliders | Uses the selected game's ALE score delta; there is no generic coordinate, room, or novelty reward panel. |
:::

:::{div} feynman-prose
The large values are not typos. Sonic's act-clear and boss terms are meant to
make finishing and fighting visible to the selection signal; Montezuma's room
bonus is meant to keep a lineage exploring. Mario's clip prevents one ordinary
frame from dominating the running reward, while its flag and area events remain
outside that clip so important transitions are not flattened away.

These weights are not interchangeable across consoles. A value of `500` means
...
defaults to `30`. Both accept integers from `1` to `120`, and `dt min` must not
exceed `dt max`. The implementation samples uniformly, inclusively, between
...
terminal event can end a hold early, so the actual number of frames is not
always the sampled number. In planner mode, the recorded edge duration is used
...
:::{div} feynman-prose
The display is a report from the current algorithm, not a single universal
camera. Wave and Graph label the screen **Best walker** and render the leading
...
map and overlays the swarm. Sonic has no complete map image in the UI: it
stitches downsampled camera tiles from the walkers, so dark territory means
“not seen by this swarm.” Montezuma builds a 24-room pyramid from valid
HUD-cropped room frames as walkers enter rooms. Generic Atari has no map panel.
...
markers, showing retained ancestry rather than a single trajectory.

The optional visit view changes the base map to grayscale and draws pooled visit
blocks with a fire scale. Never-visited blocks are transparent; walkers remain
on top. This is a count heatmap, not a prediction of where the game will go
next. It can be viewed for any solver that is counting visits in Coords mode,
...
| **Virtual reward** plot | Max and mean cloning fitness after distance/reward/visit factors are rescaled and exponentiated. It is not game score. |
...
| **Alive walkers** | Walkers that did not hit a terminal/death condition in the latest update. Wave and Graph can replace dead walkers by cloning while a live companion exists. |
...
| **Iteration** | Completed algorithm iterations. Planner execution messages do not represent a new search iteration in the plots. |
...
| **Alive** | Alive count over the current population; Graph's population can change, so read its displayed denominator rather than assuming `N`. |
| **Nodes** | Graph-only `nodes / leaves / stepped` for the latest update. |
| **Iterations/s** | Three-second sliding average of worker step messages; planner search messages contribute, while execution-only messages do not. |
...
| **Played score** | The committed emulator's display score; when a game has no separate display score, the adapter reports accumulated reward. |
...
current population-level fitness comparison does not favor it.

Planner plots are sampled on search advances. Executing a selected action
updates **Played score** and **Played frames**, but does not add a new search
point to the swarm plots. This is why the planner status can say **Playing**
while the plot lines hold still. The screen, the planner counters, and the
population plots are three synchronized but non-identical views.
:::
...
Wave and Graph stop when every current walker is dead: there is no live
companion from which to make a useful clone. A dead walker can be copied while
...
or dead player states, or game over; flag capture alone does not end this
wrapper. Sonic treats any life loss or zero lives as terminal for that walker.
...
life rule except that entering room 8 is deliberately hard terminal and earns
no room bonus. An action hold can therefore finish before its sampled `dt`.
:::
...
are intended to replay the same experiment. A seed does not promise the same
wall-clock speed, and a different browser build, ROM, observation mode, or
population changes the sequence of emulator states. A reward-term edit is
also a new reward boundary: it affects future transitions and does not
recompute history.
:::
...
:::{div} feynman-prose
When the page reports **Not cross-origin isolated**, the problem is usually the
server rather than the game. Serve the repository's browser app with
`fractal-gas-web/serve.py` and reload; opening the HTML file directly or using a
plain static server does not provide the headers needed by WebAssembly threads.
...
Atari game picker does not enable the room-aware map or room reward.
...
smaller observation is the relevant fix. If there is no map or visit control,
check that the console is Mario, Sonic, or dedicated Montezuma and that the
observation is Coords; generic Atari intentionally has neither a semantic
Coords tuple nor a map.

If all walkers die, that is a terminal population, not a rendering failure.
...
If the planner seems to “do nothing,” look at **Planner** and **Search depth**:
...
Finally, a dark Sonic map is not missing terrain; it is unexplored fog. A gray
dot is a dead walker, not a successful route. A rising Virtual reward is not a
high game score. These three interpretations account for most apparent
...
Do not compare cumulative-reward magnitudes across Mario, Sonic, generic Atari,
and Montezuma without reporting the reward adapter and slider values. The
virtual-reward plot is even less comparable: it is a population-rescaled
selection signal whose value depends on the current distance, reward, and visit
distributions.
:::
