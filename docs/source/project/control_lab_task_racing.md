...
5. Inspect the preview below **Select track**. It shows the active outline, direction, difficulty, and checkpoint count. Use **2D / 3D** to snap between overhead and angled views. Right-drag to rotate and tilt; scroll to zoom; middle-drag or Alt-drag to pan. **Follow agent** gives a close view while keeping your chosen angle. **Whole arena** and **Reset view** restore the scene's initial angle, arena center, and default zoom.
