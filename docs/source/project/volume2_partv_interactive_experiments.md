...
The main display uses two spatial coordinates and a separate time axis. Rotating a scene changes its camera, never its metric, clock or causal comparisons. The engine runs inside the browser worker through the compiled Rust/WASM library. Reference calculations also run in Rust. JavaScript supplies controls, rendering and labels; it does not introduce a second scientific implementation.
...
**Display and controls.** Use the connection-angle control from zero to four pi. The native reference assigns oriented U(1) phases and noncommuting SU(2) transports to a small loop. Display the supplied and gauge-transformed edge data, U(1) loop real/imaginary components, the normalized SU(2) Wilson trace and the invariance residual.
...
:::{div} feynman-prose
**Where and why.** Place at `sec-hypocoercivity`. It connects harmonic relaxation to the full position–velocity covariance under anisotropic noise.
...
Checkpoints preserve the optional archive alongside the numerical state. Recording coverage begins at a labeled state anchor; preceding states remain outside that coverage. Replay tests compare event identities and numerical arrays within the selected execution profile.
:::
...
A seed-7 native reference calculation with 256 samples and 32 replicas gives a useful reading example. The dimension fraction was `0.229201 ± 0.002680` (one standard error), against `8/35=0.228571`; the inverted dimension was `2.99660`. Known-density integration at contrast 0.5 returned `17.4182 ± 0.0799`, against `17.3333`. At bandwidth 0.4, the deterministic manufactured-field operator was `3.95953846`, approaching its zero-bandwidth value 4. The Monte Carlo mean was `0.8448 ± 4.1571`, with only 4.5 support points per replica on average: the uncertainty is visible because the local sample is small.

The curvature calculation separates deterministic calibration from sampling just as sharply. At K=0.2 and support scale 0.4, the kernel quadrature estimate was `0.399659` against `R=0.4`; the sampled estimate was `-4.1384 ± 5.0506`. The midpoint quadrature estimate was `0.400065`, while its sampled estimate was `31.25 ± 28.47`. These are concrete examples for the sample-size controls: both calibrated finite-radius references are close to the target, and the displayed replica uncertainty reveals how much the corresponding stochastic estimates still fluctuate.

The implementation passes 125 Rust workspace tests, 111 JavaScript lecture checks (84 covering Parts I–IV and shared behavior, and 27 specific to Part V), and 98 documentation tests. The JavaScript coverage includes control endpoints, batched checkpoint behavior, partial scene clipping, declared fitness profiles and finite-sample uncertainty comparisons. The compiled build passes browser initialization, stepping, reset and control checks for all 62 lecture demos, together with dedicated scene, import, shared-archive and mobile checks. The standalone WASM contract suite also passes.

The Sphinx documentation build succeeds. Published-site checks cover all 62 embeds, Expert Mode, lazy loading and project prefixes. The Rust workspace passes Clippy for all targets with warnings denied, and its WASM release build and WASM WebGPU configuration check pass. Adaptive sampled-fitness runs require CPU/WASM f64 execution.
...
## 7. Numerical validation and consistency

:::{div} feynman-prose
Independent checks give each numerical comparison a definite meaning. A partition must account for the observation volume exactly once. A covariance measurement must use the same normalization and elapsed time as its prediction. An estimator's uncertainty must follow the law that generates its samples. The examples below test those requirements through exact identities, analytical fixtures, refinement and independent ensembles.
:::

### Positive-volume ties and partial spacetime slices

:::{div} feynman-prose
Spacetime clipping assigns each positive-volume region to exactly one cell. Equality on a shared face is compatible with this rule because the face has zero three-volume. When two affine score fields agree throughout a tetrahedron, the smaller slot index owns that tetrahedron. A test starts two sites at `(0,0)`, moves them to `(-0.5,0.5)` and `(0.5,-0.5)`, and uses the square `[-1,1]^2` for one unit of time. The computed cell volumes sum to 4, exactly the volume of the observation slab.

Closure and geometric accuracy answer different questions. Individual moving-cell volumes should improve under refinement even when the partition closes exactly. In the moving-site test, the sum of absolute cell-volume errors at resolutions 1, 2, 4 and 8 is `1.92537`, `1.23938`, `0.357427` and `0.0902453`. The total volume remains 4 throughout. A zero closure residual therefore verifies complete accounting, while the refinement sequence measures how well the separate cell shapes approximate the moving boundaries.

The viewer intersects crossing faces and edges with the selected time plane and displays the portion on the visible side. The displayed cut therefore corresponds to a physical time, while rotation and vertical exaggeration remain view controls.
:::

### Metric conditioning, source connections and declared fitness

:::{div} feynman-prose
The symmetric two-by-two eigensolver uses scaled, stable formulas and preserves the diagonal axes. For `diag(10^20,1)`, the test recovers eigenvalues `10^20` and 1 and verifies the associated matrix functions. Computing the smaller eigenvalue by subtracting nearly equal large numbers would lose this positive value to floating-point cancellation. The stable calculation preserves the positive-definite metric and its inverse at this scale separation.

The variable-metric graph uses segment-midpoint metric quadrature for both ordinary edges and connections from sources away from grid vertices. For `g_xx(x)=1+8x`, a source at `x=0.5` and an endpoint at `x=1`, the exact segment length is `1.318305`; midpoint quadrature gives `1.322876`. The discrepancy is finite-segment quadrature error. Applying the same rule throughout the graph makes source connections participate consistently in refinement.

V-05 and V-13 differentiate a declared smooth teaching profile using recorded coordinates and companions as inputs. The conditional profile has global standardizer floor 0.1, logistic channels `2/(1+exp(-z))+10^-6`, unit powers, a quadratic objective and position-only distance. Its parameters specify the field whose Hessian is computed; coordinates alone do not determine those parameters. V-06 separately tests the metric recorded at the actual adaptive O stage.
:::

### Harmonic transients and the variance denominator

:::{div} feynman-prose
V-08 estimates population covariance with the unbiased denominator `N-1`. Dividing the centered sum of squared positions by N would instead give expectation `(N-1)/N` times the population variance for independent walkers—a systematic 3.125 percent deficit at N=32. Matching the denominator to the prediction makes the covariance residual a comparison of the same quantity.

The initial population has a transient. It starts from independent uniform positions and zero velocity, so its early covariance should be compared with the propagated initial covariance, rather than either stationary Lyapunov solution. The native reference propagates the full discrete transition and its innovation covariance. It also propagates the uniform distribution's fourth cumulant, which contributes to the exact uncertainty of the sample variance. A Gaussian approximation would discard that contribution precisely where the initial distribution matters most.

The harmonic test uses stiffness `diag(2,3)`, metric `diag(3,1)`, temperature 0.4, friction 1, timestep 0.04, independent initial positions uniform on `[-1.8,1.8]` in each coordinate and initial velocity zero. An independent ensemble of 512 runs with 32 walkers at time 0.96 gave mean position sample variance `0.208685`, against the exact transient prediction `0.207616`. The standard deviation of the sample variance across runs was `0.039841`, against its prediction `0.039810`. These are uncertainties of one run's sample variance; the SEM of the 512-run mean is smaller by the square root of 512. The card shows the exact transient and its pointwise uncertainty bands, with the stationary references retained for the long-time comparison.
:::

### Empty neighborhoods, variance rates and replica identities

:::{div} feynman-prose
In V-16, small support and finite sampling can make every observed replica return zero. An empirical variance calculation then also returns zero. Treating that as a resolved uncertainty interval would suggest high precision exactly when no local information was collected. An independent integration of the square of the single-draw estimator supplies the sampling uncertainty. Its variance, MSE and SEM remain positive even when all observed supports are empty. The empirical curves and zero-support fraction remain visible, so the student can see both the realized experiment and its sampling risk.

For the chosen manufactured field, the gradient at the origin is zero. Its leading difference is quadratic, and the sharp variance scale is therefore `1/(N epsilon^3)` in three spacetime dimensions. The chapter's more general `1/(N epsilon^5)` upper bound allows a nonzero gradient and remains valid. At bandwidth 1 with 128 replicas, N=1024 gave observed single-run variance `6.61668` against independently integrated variance `7.52143`; N=4096 gave `1.57694` against `1.88036`. The reference decreases exactly fourfold when N increases fourfold. The corresponding ensemble means were `3.52184` and `3.56649`, against finite-bandwidth expectation `3.74712`, with predicted SEMs `0.242407` and `0.121204`. The difference between `3.74712` and the limiting wave operator 4 is deterministic bias, measured by the separate bandwidth plot.
...
:::{div} feynman-prose
V-19 preserves assigned replica indices when an ordering fraction cannot be inverted within the admitted dimension interval. It reports the coverage of resolved inversions, so the fraction and dimension series identify the same runs even when some dimension estimates are unavailable. Dimension-dependent captions distinguish spacetime dimensions 2, 3 and 4 from their spatial coordinates and identify the projection used in dimension four. The calibrated ordering fractions are `1/2`, `8/35` and `1/10`.

The count card displays the uncertainty of both its mean and variance estimates. Mean error bars use independent-replica SEM. Error bars on the unbiased sample variance use the Poisson or binomial fourth central moment, allowing a finite ensemble's variance discrepancy to be assessed on its own statistical scale. For example, the compiled seed-7 fixed-count check with N=1024 and 128 replicas gives region sample variance `230.378`, against the binomial value 192. Its independently predicted uncertainty is `24.0904`, so the visible 20 percent excess is approximately 1.59 standard deviations of the sample-variance estimator, rather than a changed binomial law.
:::
...
:::{div} feynman-prose
The Fractal Set Spin(2) codec computes the larger spinor component stably and obtains the smaller one from the product identity `2uv=y`. Near the positive x axis, this avoids subtracting the vector's x coordinate from its nearly equal norm. For `(1,10^-9)`, the codec retains the transverse component needed to reconstruct the vector. Both the independent analysis codec and the archive codec have near-axis tests.

Instantaneous-interaction and historical-source relations carry source-minus-recipient position displacement and its Spin(2) encoding when the recorded source is available. IA relations retain their separate displacement semantics. Historical geometry uses the historical source coordinates. Missing coverage is an explicit absence.

Archive import requires mandatory stages in order, including validated `post_clone`, unique stage labels, monotone versions, consistent incarnation counters, exact covered source version/generation/eligibility and positive finite donor scores. These requirements make the individually plausible arrays describe one coherent trajectory. At the anchor-to-first-step and final-to-next-step boundaries, the same event identity and observation version cannot carry contradictory coordinates, field schemas or opaque state. Recipient generations must agree and versions cannot decrease. A declared extraction that advances the observation version remains valid; refreshed reward provenance or validity is distinguished from a changed coordinate record. Together these checks establish which recorded state and historical source each relation identifies.
:::
