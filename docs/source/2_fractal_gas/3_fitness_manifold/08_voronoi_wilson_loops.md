...
**Prerequisites**: {doc}`01_emergent_geometry`, {doc}`02_scutoid_spacetime`, {doc}`/source/2_fractal_gas/2_fractal_set/03_lattice_qft`
...
matches the Fractal Set $SU(2)$ construction (see {doc}`/source/2_fractal_gas/2_fractal_set/03_lattice_qft`) but
