...
:::{div} feynman-prose
**Three levels of analysis:** The discrete transition, the mean-field particle limit, and a continuous-time equation answer different questions. Their connection uses the transition-rate, coupling, and boundary estimates in the convergence chapters. A WFR interpretation requires an additional metric-evolution argument.
:::

:::{div} feynman-prose
**Forced cloning and population size:** With at least one alive companion, the default revival rule forces every dead walker to clone. The swarm retains $N$ entries; subsequent boundary checks and kinetic steps can change which entries are alive. The all-dead configuration follows the specified cemetery rule.
:::
...
:::{div} feynman-prose
This chapter explains what the algorithm does and why its components are present. The formal specification is {doc}`02_fractal_gas_latent`; the independent convergence arguments are developed in the convergence chapters.

Basic optimization and probability are sufficient prerequisites. For the generic mathematical setup, continue to {doc}`../convergence_program/01_fragile_gas_framework`. The finite-particle, mean-field, and entropy results state the hypotheses needed for each implementation.
:::
...
- Momentum-conserving cloning and the conditional revival guarantee
...
# 7) Repeat; track N slots, with the declared cemetery rule for all-dead states
```
...
Fractal Gas step overview: selection, fitness, cloning, kinetics, and repeat with conditional revival of N slots.
:::
...
Dual-channel fitness pipeline: reward and diversity are standardized, passed through bounded maps, and multiplied into $V_{\text{fit}}$.
:::
...
:alt: Diagram of cloning decision, jittered position updates, group velocity blending, and conditional revival of dead walkers.
:width: 95%
:align: center

Cloning mechanics: fitter companions attract low-fitness walkers, jitter maintains diversity, and revival preserves population slots when an alive set exists.
:::
...
:::{div} feynman-prose
Increasing the number of walkers and decreasing the time step are separate limits. A particle coupling controls the first. A generator expansion, including the acceptance-rate scaling and boundary law, controls the second. Under those estimates, a density equation describes the limiting law and makes its transport and selection contributions explicit.

The derivation is {doc}`../convergence_program/08_mean_field`, and the existence, uniqueness, and particle-limit arguments are in {doc}`../convergence_program/09_propagation_chaos`. Their conclusions transfer to the algorithm when its actual transition and companion kernels satisfy the stated hypotheses.
:::
...
For illustration, an overdamped Euclidean diffusion combined with a specified local replicator reaction has the following equation. The actual Fractal Gas companion kernel generally gives a nonlocal reaction instead; its derivation is in {doc}`../convergence_program/08_mean_field`. WFR is a metric setting for certain transport-and-reaction evolutions {cite}`liero2018optimal,chizat2018interpolating`:
...
:::{div} feynman-prose
In this illustrative equation, diffusion spreads probability and the drift follows the negative gradient of $\Phi$. The replicator term increases density where fitness exceeds its current mean. Subtracting that mean makes the reaction mass-neutral when $\rho$ is normalized.

These effects explain the roles of exploration and selection. Their balance depends on the potential and reaction law; deterministic drift can approach local minima, and diffusion on an unbounded space does not approach a uniform probability law. A convergence rate follows from an estimate for the specified equation, such as the hypocoercive entropy proofs in {doc}`../convergence_program/15_kl_convergence`.
:::

### Mean-Field Limit

:::{div} feynman-prose
For a matched particle coupling, let $\varepsilon_N(t)$ be the mean squared error between interacting particles and independent copies of the limiting process. The empirical estimate is

$$
\mathbb E W_2(\mu_N(t),\mu_t)
\leq\sqrt{\varepsilon_N(t)}
+\mathbb E W_2(\bar\mu_N(t),\mu_t),
$$

where $\bar\mu_N$ is the empirical measure of those independent copies. The first term measures interaction error; the second is sampling error and retains its dimension dependence. The full proof is {prf:ref}`thm-mean-field-limit-informal`, with the model-specific coupling in {doc}`../convergence_program/09_propagation_chaos`.
:::
...
:::{div} feynman-prose
The Fractal Gas combines companion selection, reward-and-diversity fitness, cloning, and kinetic motion. The operational rules specify a Markov transition once their kernels and boundary behavior are fixed. The default revival rule forces cloning from the available alive set; the all-dead configuration requires its specified cemetery rule.

The analytic conclusions are developed in separate, connected steps:

- {doc}`../convergence_program/06_convergence`: finite-particle recurrence and QSD convergence for the stated model.
- {doc}`../convergence_program/09_propagation_chaos`: mean-field well-posedness, stationary identification, and particle comparisons.
- {doc}`../convergence_program/15_kl_convergence`: full-gradient LSI and hypocoercive entropy proofs for identified laws and generators.
- {doc}`../convergence_program/17_geometric_gas`: geometric coefficient, stability, and entropy estimates with their spectral and derivative conditions.

The specification tells you what is computed. These estimates tell you which distribution is controlled, which parameters enter the rate, and which constants remain independent of population size.
:::

:::{seealso}
:class: feynman-added

- {doc}`02_fractal_gas_latent`: latent-state algorithm and mathematical definitions.
- {doc}`../convergence_program/01_fragile_gas_framework`: generic assumptions and transition rules.
- {doc}`../convergence_program/08_mean_field`: continuous forward equation and its connection to discrete updates.
:::
