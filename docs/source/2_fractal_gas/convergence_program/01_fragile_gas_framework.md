...
:::{div} feynman-prose
This chapter defines the measurement, selection, cloning, perturbation, and
status kernels of the Fragile Gas. Its quantitative assumptions expose how
...
coupling of those laws. For the fixed-$N$ Gaussian realization, the chapter
proves local total-variation and Wasserstein continuity with constants that
can depend on $N$.

Guaranteed revival restores dead proposal slots when an alive companion is
available and the revival condition holds. A later perturbation and validity
test can still kill walkers. The finite-particle drift and QSD convergence
results are in {doc}`03_cloning`, {doc}`05_kinetic_contraction`, and
{doc}`06_convergence`; the entropy and functional-inequality proofs are in
{doc}`15_kl_convergence`.
:::

## 1. Introduction

### 1.1. The update and the quantities being compared

:::{div} feynman-prose
A swarm update begins with information: rewards, companion distances, and
alive statuses. Standardization converts that information into fitness.
Selection then chooses replacements, perturbation moves walkers, and the
status rule records which proposed states remain valid. To analyze the update,
we specify each of these operations as a map or a probability kernel.

Suppose two identical initial swarms each draw their own independent noise.
Their next states will usually differ. This does not signal a discontinuity
of the algorithm: the two output laws are identical. It tells us that the
expected distance between independent draws and the distance between their
probability laws are different quantities. The chapter keeps both estimates,
using the coupling appropriate to each.

For independent outputs, the composite estimate retains its noise offset and
the fractional powers generated by successive stages. For the Gaussian
transition law, finite-history probabilities and shifted Gaussian densities
give a local total-variation bound. A common-part coupling and moment estimates
then give local Wasserstein continuity. The proof is in
{prf:ref}`prop-w2-bound-no-offset`.
:::

### 1.2. Reading the parameters

:::{div} feynman-prose
A regularity constant measures how much one stage can amplify a change in its
input. Following it through the pipeline identifies which estimates become
large when a standardization floor is small or the reward varies rapidly.
...
forever. Finite-horizon extinction estimates retain the probability of the
later killing stage and its dependence on the swarm configuration.
:::

### 1.3. Reading guide

:::{div} feynman-prose
Sections 2–8 introduce the states, assumptions, environment, noise, measurement,
and companion laws. Sections 9–13 develop rescaling, distance measurements,
standardization, and fitness. Sections 14–17 treat perturbation, status,
cloning, and single-survivor revival. Sections 18–19 compose and iterate the
update. Sections 20–21 give concrete choices and collect the constants.

For an implementation, begin with the update definition and trace each of its
choices back to the corresponding kernel. For a proof, retain its coupling,
state-space domain, and normalization along with its constants. The Euclidean
and latent specifications are given in {doc}`02_euclidean_gas` and
...
On a measurable state space $(E,\mathcal E)$, a Markov kernel $P$ satisfies
$P(x,E)=1$, countable additivity in its second argument and measurability in
its first. A killed kernel $K$ is sub-Markov: $K(x,E)\le1$. A cemetery state
accounts for the missing mass. For an initial law $\mu$ with
$\mu K^n\mathbf1>0$, its survival-conditioned law is
$\mu K^n/(\mu K^n\mathbf1)$. This normalization is generally nonlinear in $\mu$.
:::

:::{prf:definition} Foster--Lyapunov drift
:label: def-foster-lyapunov

For $V:E\to[1,\infty)$, $0<a<1$, $b<\infty$ and a measurable set $C$, the
inequality $PV\le aV+b\mathbf1_C$ is a geometric drift condition. Applying
it to a killed kernel gives a bound on $KV$; conclusions about conditioned
laws also require survival and minorization hypotheses. Iteration of the
weaker inequality $PV\le aV+b$ gives
$P^nV\le a^nV+b(1-a^n)/(1-a)$.
:::

:::{prf:definition} Geometric ergodicity of a conservative kernel
:label: def-geometric-ergodicity

A conservative kernel $P$ with invariant probability $\pi$ is geometrically
ergodic in the $V$-norm if there are $M<\infty$ and $r\in(0,1)$ such that
$\|P^n(x,\cdot)-\pi\|_V\le M V(x)r^n$, where
$\|\eta\|_V=\sup_{|f|\le V}|\eta f|$.
For a killed kernel, exponential convergence of the normalized laws to a
quasi-stationary distribution is a separate property.
:::

:::{prf:definition} Wasserstein distance of order two
...
$$
where $\Pi(\mu,\nu)$ denotes couplings with the prescribed marginals.
A bound on a variance proxy above $W_2^2$ is an upper bound; contraction in
$W_2$ requires an estimate proportional to the initial $W_2$ distance.
:::

:::{prf:definition} Kinetic Langevin operator
:label: def-langevin-operator
...
Used in {prf:ref}`thm-total-error-status-bound`.
:::
:::{prf:proof}
**Proof.**
...
This lemma supplies the cubic patch in {prf:ref}`def-asymmetric-rescale-function`.

For every $z_{\max}>1$, there is a unique cubic polynomial $P$ satisfying the four endpoint value and derivative conditions in that definition.
:::
:::{prf:proof}
...
Let the cubic polynomial patch $P(z)$ be defined on the interval $[z_{\max}-1, z_{\max}]$. Introduce $s=z-(z_{\max}-1)$, so $s\in[0,1]$. In this coordinate write $P(z)=Q(s)=As^3+Bs^2+Cs+D$.
...
The **Canonical Logistic Rescale Function ({prf:ref}`axiom-rescale-function`)** defined in {prf:ref}`def-canonical-logistic-rescale-function-example` satisfies all four conditions of the **Axiom of a Well-Behaved Rescale Function** ({prf:ref}`axiom-rescale-function`).
:::
:::{prf:proof}
**Proof.**
We verify the four conditions of {prf:ref}`axiom-rescale-function` directly.
...
$$
2.  **Bound the Positional Component:** From {prf:ref}`lem-sub-stable-positional-error-bound`, the total squared positional error component is bounded by:
...
$$
3.  **Bound the Structural Component:** From {prf:ref}`lem-sub-stable-structural-error-bound`, the total squared structural error component is bounded by:
...
**Proof.**
The total error for a single stable walker ({prf:ref}`def-walker`) is first decomposed into a positional component and a structural component. The squared L2-norm of the total error over all stable walkers is then bounded by combining the established bounds for the sum of the squares of these individual components.
1.  **Decomposition of Total Stable Error:** From {prf:ref}`lem-sub-stable-walker-error-decomposition`, the total squared error for stable walkers is bounded by twice the sum of the squared positional and structural error components:
...
Let $a=\sigma'_{\min}>0$ and $s(V)=\sqrt{V+a^2}$ for $V\geq0$. Then

$$
|s'(V)|\leq\frac1{2a},\qquad
|s''(V)|\leq\frac1{4a^3},\qquad
|s'''(V)|\leq\frac3{8a^5}.
$$
...
$$
|s^{(n)}(V)|=\frac{(2n-3)!!}{2^n}(V+a^2)^{1/2-n}
\leq\frac{(2n-3)!!}{2^n a^{2n-1}},
$$
...
:::{prf:proof}
Differentiate the power $(V+a^2)^{1/2}$ repeatedly. Its coefficient is
$\prod_{j=0}^{n-1}(1/2-j)$, whose magnitude is $(2n-3)!!/2^n$.
The negative exponent makes the magnitude largest at $V=0$.
:::
...
where $L_{\mu,M}$ is the axiomatic value Lipschitz function for the mean from {prf:ref}`def-swarm-aggregation-operator-axiomatic` (explicit expressions for the empirical aggregator ({prf:ref}`lem-empirical-aggregator-properties`) appear in {prf:ref}`lem-empirical-aggregator-properties`), and $L_{\sigma',M}$ is the derived Lipschitz constant for the regularized standard deviation, given by:
...
:::{prf:proof}
**Proof.**
The bound for the mean $\mu$ is a direct application of the axiom in {prf:ref}`def-swarm-aggregation-operator-axiomatic`. The bound for $\sigma'$ is derived by composition. $\sigma'(\mathcal{S}, \mathbf{v})$ is the composition of the variance function $\text{Var}(\mathbf{v}) = m_2(\mathcal{S}, \mathbf{v}) - \mu(\mathcal{S}, \mathbf{v})^2$ and the smoothed function $\sigma'_{\text{reg}}(V)$.
...
:::{prf:proof}
**Proof.**
The squared norm is $k \cdot (\mu_2 - \mu_1)^2 / (\sigma'_1)^2$. From the aggregator axiom ({prf:ref}`def-swarm-aggregation-operator-axiomatic`), $(\mu_2 - \mu_1)^2 \leq (L_{\mu,M}(S))^2 \|v_1 - v_2\|_2^2$. Combining this with the lower bound on $\sigma'_1$ gives the final result.
**Q.E.D.**
:::
...
1.  **Start with the Decomposed Error Bound.**
    From {prf:ref}`lem-sub-value-error-decomposition`, we have a deterministic bound on the squared error for any specific realization of $v_1$ and $v_2$:
...
:::{prf:definition} Structural standardization coefficients
:label: def-structural-error-coefficients

Let $J=\mathcal A_1\cap\mathcal A_2$, $|v_i|\le V_{\max}$, and
$s_r=\sigma'_r\ge m>0$. Suppose the aggregator bounds give
$|\mu_1-\mu_2|\le L_\mu n_c$ and $|s_1-s_2|\le L_s n_c$.
For the following structural estimates set
$$
C_{S,\mathrm{direct}}=\frac{4V_{\max}^2}{m^2},\qquad
C_{S,\mathrm{indirect}}=|J|\left(\frac{L_\mu}{m}
 +\frac{2V_{\max}L_s}{m^2}\right)^2.
$$
These constants depend on the stated aggregator bounds and the two alive sets.
:::
...
For $i\in J$ write
$$
\frac{v_i-\mu_1}{s_1}-\frac{v_i-\mu_2}{s_2}
=\frac{\mu_2-\mu_1}{s_1}
 +\frac{(v_i-\mu_2)(s_2-s_1)}{s_1s_2}.
$$
The hypotheses in {prf:ref}`def-structural-error-coefficients` bound its
absolute value by $n_c(L_\mu/m+2V_{\max}L_s/m^2)$.
Squaring and summing over $J$ proves the indirect bound. On the symmetric
difference of the alive sets the direct bound follows from
{prf:ref}`lem-sub-direct-structural-error`. The supports are disjoint, so their
...
:::{prf:proof}
**Proof.**
The proof follows from decomposing the total error using an intermediate vector and then applying the triangle inequality. Let the intermediate vector be $z_{\text{inter}} := z(S_1, v_2, M)$, which uses the second raw value ({prf:ref}`def-raw-value-operator`) vector with the first swarm ({prf:ref}`def-swarm-and-state-space`)'s structure.
The total error vector is $z_1 - z_2 = (z_1 - z_{\text{inter}}) + (z_{\text{inter}} - z_2)$.
...
The proof of the decomposition is a direct algebraic manipulation.
1.  **Start with the Definition of the Error.**
    The total error is $\Delta\mathbf{z} = z_1 - z_2 = (v_1 - \mu_1) / \sigma'_1 - (v_2 - \mu_2) / \sigma'_2$.
2.  **Decomposition.**
    We add and subtract terms to isolate the desired components.
...
:::{prf:proof}
**Proof.**
The proof proceeds by bounding the squared L2-norm of each of the three components from the algebraic decomposition in {prf:ref}`lem-algebraic-value-error-decomposition` and then summing them.
...
4.  **Combine the Bounds:**
    Substituting the bounds for each of the three components into the inequality from {prf:ref}`lem-algebraic-value-error-decomposition` ($\|\Delta\mathbf{z}\|_2^2 \leq 3(\|\Delta_{\text{direct}}\|_2^2 + \cdots)$), and factoring out the common term $\|v_1 - v_2\|_2^2$, yields the final result. The sum of the coefficients for each component, multiplied by 3, constitutes the **Total Value Error Coefficient**, $C_{V,\text{total}}(S)$. Since all constituent parts are finite for a given state **S**, $C_{V,\text{total}}(S)$ is a finite constant.
**Q.E.D.**
:::
...
:::{prf:proof}
**Proof.**
The proof proceeds by decomposing the total structural error vector $\Delta\mathbf{z} = z(S_1, v) - z(S_2, v)$ into two orthogonal components: a "direct" error from walker ({prf:ref}`def-walker`)s whose status changes, and an "indirect" error affecting walkers whose status is stable.
1.  **Decomposition of Structural Error:** The N-dimensional error vector $\Delta\mathbf{z}$ is partitioned based on walker ({prf:ref}`def-walker`) indices. The squared norm is the sum of the squared norms over these disjoint sets:
...
2.  **Bound the Components.**
    The Perturbation Operator ({prf:ref}`def-perturbation-operator`) does not alter the survival status of any walker ({prf:ref}`def-walker`), so the status change term is deterministic. The output positional displacement is bounded by {prf:ref}`lem-sub-perturbation-positional-bound-reproof`.
3.  **Construct a Probabilistic Bound.**
    The random variables $\Delta_{\text{pert}}^2(\mathcal{S}_1)$ and $\Delta_{\text{pert}}^2(\mathcal{S}_2)$ are independent. We use the **union bound** to establish a simultaneous high-probability bound for both terms, allocating a failure probability of $\delta' = \delta/2$ to each. From {prf:ref}`lem-sub-probabilistic-bound-perturbation-displacement-reproof`, both bounds hold simultaneously with probability at least $1-\delta$.
...
:class: note
This concentration bound relies on Assumption A (in‑step independence) and on the with‑replacement sampling policy in §7.1.1. Implementations that use within‑step dependence (e.g., systematic resampling with a shared uniform) violate these assumptions; a dependent bounded-differences estimate must then be established for that sampling policy.
:::
...
:::{div} feynman-prose
The deterministic status map changes abruptly at the validity boundary. After perturbation, its survival probabilities can vary continuously with the input under {prf:ref}`axiom-boundary-regularity`. The following independent-output estimate combines changes in those probabilities with the variance of the sampled statuses. Its variance term remains even when the inputs agree.
:::
...
:::{div} feynman-prose
The cloning transition averages over distance measurements, comparison companions, and threshold decisions. Its total cloning probability is a deterministic function of the input state. Bounds for changes in that probability are one ingredient in the output-displacement estimate. Continuity of the entire transition law also accounts for where each selected branch sends the walkers.
:::
...
For two swarm states $\mathcal S_1,\mathcal S_2$ with nonempty companion sets, the total expected cloning probabilities satisfy the following decomposition:
...
:::{prf:lemma} Structural cloning-probability error
...
**Proof.**
Apply the deterministic bound of {prf:ref}`thm-deterministic-potential-continuity` under the specified coupling and take expectations, using the preceding mean-square bounds for the input measurements. The explicit form of $F_{\text{pot}}$ is constructed from the composition of the mean-square continuity bounds of all preceding operators.
**Q.E.D.**
:::
#### 15.2.7. Bounding the Value Component of Cloning Probability Error
:::{prf:lemma} Value cloning-probability error
...
The expected squared displacement between these independent outputs has the following bound, including its intrinsic stochastic offset:
...
:::{prf:definition} Cloning displacement coefficients
...
3.  **Substitute the Bound for the Fitness Potential Error ($F_{\text{pot}}$).**
    The crucial step is to substitute the bound for the **Expected Squared Potential Error Bound** ($F_{\text{pot}}$) from {prf:ref}`thm-potential-operator-is-mean-square-continuous`. $F_{\text{pot}}$ is itself a function of the input displacement components: $F_{\text{pot}}(S_1, S_2) = F_{\text{unstable}} + F_{\text{stable}}$, where $F_{\text{stable}}$ is bounded by the mean-square errors of the standardization pipelines for reward and distance. The distance standardization error ($\mathbb{E}[\|\Delta\mathbf{z}_d\|^2]$) from {prf:ref}`thm-distance-operator-mean-square-continuity` contains a term proportional to $n_c^2$.
...
4.  **Relate Displacement Components to $V_{\text{in}}$.**
    ...
    *   $n_c^2 \le \left(\frac{N}{\lambda_{\text{status}}}\right)^2 \cdot V_{\text{in}}^2$
    Substituting these into the bound for $F_{\text{pot}}$ shows that $F_{\text{pot}}$ is bounded by a quadratic function of $V_{\text{in}}$: $F_{\text{pot}} \leq B_2 V_{\text{in}}^2 + B_1 V_{\text{in}} + B_0$.
5.  **Finalize the Bound on the L1-Norm.**
    ...
    Combining all terms, the total L1-norm $\|\Delta\overline{\mathbf{P}}\|_1$ is bounded by an expression of the form $C'_P V_{\text{in}} + H'_P \sqrt{V_{\text{in}}} + K'_P$. Absorbing the term **2N** into the constant offset gives the final result as stated in the sub-lemma.
**Q.E.D.**
:::
...
:::{prf:lemma} Final positional displacement
...
**Q.E.D.**
:::
#### 17.2.4. Composite independent-output displacement
...
:::
:::{prf:theorem} Composite displacement estimate
:label: thm-swarm-update-operator-continuity-recorrected

Use the independent output coupling of
{prf:ref}`thm-cloning-transition-operator-continuity-recorrected` in the
$k\ge2$ regime. Set $V=d_{\mathrm{Disp},\mathcal Y}(\mathcal S_1,\mathcal S_2)^2$,
$a=C_{\mathrm{clone},L}$, $b=C_{\mathrm{clone},H}$,
$c=K_{\mathrm{clone}}$, and $\alpha=\alpha_B\in(0,1]$. Define
...
\le 3aV+3bV^{1/2}+Aa^\alpha V^\alpha
+Ab^\alpha V^{\alpha/2}+3c+Ac^\alpha+K_0.
$$
...
:::{prf:definition} Composite displacement coefficients
:label: def-composite-continuity-coeffs-recorrected

With the preceding notation, define

$$
C_{\Psi,L}=3a,\quad C_{\Psi,H}=3b+Aa^\alpha+Ab^\alpha,\quad
K_\Psi=3c+Ac^\alpha+K_0,\quad
p_-=\alpha/2,\quad p_+=\max(1/2,\alpha).
$$

A form that retains all small-displacement powers is

$$
\mathbb E d_{\mathrm{out}}^2\le C_{\Psi,L}V+K_\Psi+
C_{\Psi,H}\begin{cases}V^{p_-},&0\le V\le1,\\V^{p_+},&V>1.\end{cases}
$$

Here $\alpha_H^{\mathrm{global}}=p_+$ denotes only the large-displacement
exponent. The $V^{\alpha/2}$ term controls the small-displacement estimate.
:::

#### 17.2.4.2. Proof of the composite displacement estimate
:::{prf:proof}
:label: proof-composite-continuity-bound-recorrected

Let $Z=N^{-1}\Delta_{\mathrm{pos},\mathrm{clone}}^2$. The cloning estimate
gives $\mathbb E Z\le B(V)$. Conditional on the intermediate states,
{prf:ref}`lem-final-positional-displacement-bound` and
{prf:ref}`lem-final-status-change-bound` give

$$
\mathbb E d_{\mathrm{out}}^2
\le3\mathbb E Z+K_0+
\lambda_{\mathrm{status}}N^{\alpha-1}
C_{\mathrm{status},H}(\mathbb E Z)^\alpha.
$$
...
piecewise form without discarding any term.
:::

#### 17.2.4.2a. Lemma: Subadditivity of Fractional Powers
:::{prf:lemma} Subadditivity of fractional powers
...
:::{prf:lemma} Global unification of fractional powers
...
The bound $A_\Sigma(1+V^{p_{\max}})$ controls growth for all $V\geq0$.
For a vanishing small-displacement term, retain the smallest exponent on
$[0,1]$ and the largest exponent on $[1,\infty)$, as in
{prf:ref}`def-composite-continuity-coeffs-recorrected`. The composite calculation
contains $V^{\alpha_B/2}$, so its local exponent is $\alpha_B/2$.

:::
(subsec-w2-coupling-offset-removal)=
#### 17.2.4.4. Coupling the transition laws

:::{div} feynman-prose
To compare two transition laws, couple their common mass identically and control the unmatched mass by the output diameter or moments. This gives a bound that vanishes as the laws approach one another in total variation. It uses a different coupling from the independent-output calculation above.
:::
...
:::
:::{prf:proposition} Coupling bound for the transition law
:label: prop-w2-bound-no-offset

Let $\mu=\Psi(\mathcal S_1,\cdot)$ and $\nu=\Psi(\mathcal S_2,\cdot)$,
with $\varepsilon=\|\mu-\nu\|_{\rm TV}=\sup_A|\mu(A)-\nu(A)|$.
For an output metric of diameter $D_o$,

$$
W_2^2(\mu,\nu)\le D_o^2\varepsilon.
$$

On an unbounded output space, fix $o$ and write
$M_4(\mu)=\int d(o,z)^4\mu(dz)$. Then
...
In particular, the fixed-$N$ Gaussian realization with positive fixed
perturbation variance, the regularized measurement maps of this chapter,
and its finite companion and cloning choices is locally $W_2$ continuous
on each status stratum. Its transition total variation is locally bounded
by $C_N\|x-y\|$. The constant in this one-step estimate may depend on $N$;
population-uniform long-time estimates are established in
{doc}`06_convergence` and {doc}`13_quantitative_error_bounds`.
:::

:::{prf:proof}
Couple the common measure $\mu\wedge\nu$ identically and couple the two
residual measures arbitrarily. The mismatch event has probability
$\varepsilon$. This gives the bounded-diameter estimate. In the unbounded
case, Cauchy--Schwarz and
$d(z,w)^4\le8\{d(z,o)^4+d(w,o)^4\}$ give

$$
\mathbb E d(Z,W)^2\le
\sqrt{\mathbb E d(Z,W)^4}\sqrt{\Pr(Z\ne W)}
\le\sqrt{8\{M_4(\mu)+M_4(\nu)\}}\sqrt\varepsilon.
$$

For the Gaussian realization, enumerate the finitely many distance
companions, cloning companions, and clone/persist choices by $h$.
On a fixed status stratum their probabilities $p_h(x)$ are locally
Lipschitz: distances are Lipschitz, the regularized standard deviation is
bounded away from zero, the rescaling maps are Lipschitz on their ranges,
and the clipped clone probabilities have positive denominators.
Products and finite sums preserve this property. The same argument applies
to normalized positive companion weights; sequential histories use the
conditional remaining-candidate probabilities.

Conditional on $h$, the pre-status positions have a Gaussian law
$G_h(x)$ with mean $A_hx$ and a fixed covariance $\Sigma_h\succeq\sigma^2 I$.
The independent final perturbation ensures this lower bound even for
persistent walkers. If $g$ is the standard Gaussian density, integration
of its derivative along a translation gives

$$
\frac12\int|g(z-u)-g(z)|\,dz
\le\frac12\|u\|\,\mathbb E|Z_1|
\le\|u\|.
$$

Consequently
$\|G_h(x)-G_h(y)\|_{\rm TV}\le
\|\Sigma_h^{-1/2}A_h(x-y)\|$.
The deterministic status update is a measurable pushforward and cannot
increase total variation. For the mixture,

$$
\|\Psi(x,\cdot)-\Psi(y,\cdot)\|_{\rm TV}
\le\frac12\sum_h|p_h(x)-p_h(y)|+
\sum_h p_h(y)\|G_h(x)-G_h(y)\|_{\rm TV}
\le C_N\|x-y\|.
$$

Gaussian fourth moments are locally bounded in the input positions;
statuses contribute a bounded amount to the metric. The preceding
coupling estimate therefore proves local $W_2$ continuity. In the
quotient metric, apply the estimate after a minimizing relabeling, using
equivariance of the update. The discrete status cost ensures that
sufficiently close configurations have the same status stratum.
:::

:::{prf:remark} Independent outputs and coupled laws
:label: rem-remark-context-5042

The coefficients in {prf:ref}`def-composite-continuity-coeffs-recorrected`
bound independently sampled outputs and retain their stochastic offset.
The coupling in {prf:ref}`prop-w2-bound-no-offset` instead uses the common
part of the transition laws. Its constants follow from variation and
moment estimates. Equality of two outputs driven by the same random seed
at equal inputs alone supplies no quantitative modulus at unequal inputs.
:::
(subsec-measurability-markov-kernel)=
...
The coupling in {prf:ref}`prop-w2-bound-no-offset` compares the resulting transition measures and preserves each marginal update law.
**Q.E.D.**
:::
:::{prf:remark}
...
:::{prf:proof}
Let $f\in C_b(\Sigma_N)$. The perturbed state is obtained by sampling positions independently according to the product density $\prod_i p_\sigma(x_i,\cdot)$ while statuses remain fixed. For a convergent sequence of inputs the normalized densities converge pointwise. If $q_n\to q$ are these product densities, then $\min(q_n,q)\to q$ and $0\le\min(q_n,q)\le q$. Dominated convergence gives $\int|q_n-q|=2-2\int\min(q_n,q)\to0$. Thus the difference of expectations is at most $\|f\|_\infty\int|q_n-q|$ and tends to zero.
**Q.E.D.**
:::
...
:::
:::{admonition} Coupling and the marginal update law
:class: feynman-added note

A coupling changes the relation between two compared runs while preserving
each run's marginal transition law. The proof in
{prf:ref}`prop-w2-bound-no-offset` matches the common part of the two output
laws and bounds their residuals. Within a single run, the random-input
conventions of Assumption A remain those used to define its kernel.
:::
...
:::{div} feynman-prose
The following choices give concrete noise and aggregation kernels whose constants can be evaluated with the preceding lemmas. Their applicability depends on the stated domain and boundary conventions. A compact valid region can be embedded in an unbounded proposal space; a bound using its diameter applies only to positions restricted to that region. The local Gaussian continuity proof instead uses output moments for unbounded proposals.
:::
...
:::{div} feynman-prose
For an untruncated Gaussian, the proposal positions live in ambient Euclidean space; any bounded set used above is the working region for the corresponding estimate. A truncated or projected proposal defines a different kernel whose normalization must be retained. Use the matching kernel when substituting the canonical constants.
:::
...
:::{div} feynman-prose
The independent-output estimate keeps a stochastic offset. Its other terms
change their relative size at input squared displacement one: for a small
number, the smaller fractional exponent gives the larger value; for a large
number, the larger exponent does. The exact composition therefore has two
regimes. The following summary uses the coefficients already proved in
{prf:ref}`thm-swarm-update-operator-continuity-recorrected`.
:::

:::{prf:remark} Canonical substitution in the composite displacement estimate

Set $V=d_{\mathrm{Disp},\mathcal Y}(\mathcal S_1,\mathcal S_2)^2$ and

$$
a=C_{\mathrm{clone},L},\quad b=C_{\mathrm{clone},H},\quad
c=K_{\mathrm{clone}},\quad \alpha=\alpha_B,
$$
...
With these normalizations, the proved estimate is

$$
\mathbb E d_{\mathrm{out}}^2\leq
3aV+3bV^{1/2}+Aa^\alpha V^\alpha+Ab^\alpha V^{\alpha/2}
+3c+Ac^\alpha+K_0.
$$

Equivalently, with $C_{\Psi,L}=3a$,
$C_{\Psi,H}=3b+Aa^\alpha+Ab^\alpha$, and
$K_\Psi=3c+Ac^\alpha+K_0$,

$$
\mathbb E d_{\mathrm{out}}^2\leq C_{\Psi,L}V+K_\Psi+
C_{\Psi,H}
\begin{cases}
V^{\alpha/2},&0\leq V\leq1,\\
V^{\max(1/2,\alpha)},&V>1.
\end{cases}
$$

The canonical stage constants are substituted into $a,b,c,A,K_0$ with the
same normalized or unnormalized conventions used in their defining lemmas.
The separate transition-law estimate is
{prf:ref}`prop-w2-bound-no-offset`; it uses total variation and output moments.
:::

:::{div} feynman-prose
Changing a parameter can affect several constants at once. A smaller
perturbation scale decreases its displacement moment, while its Gaussian
translation and boundary-probability constants can increase. Likewise, a
larger standardization floor reduces some amplification factors while changing
the resulting fitness signal. Evaluate the complete bound for the intended
kernel and domain.
:::

## Dependent bounded differences (optional)

:::{admonition} Random-input assumptions in concentration bounds
:class: feynman-added note

The bounded-difference estimates use the independent primitive random inputs
specified for the update. If an implementation changes their dependence, its
concentration proof must use an inequality whose hypotheses match that law.
Dependence between derived quantities alone does not invalidate bounded
differences when those quantities are functions of independent inputs; the
required sensitivity bounds must include the entire dependent update.
:::
